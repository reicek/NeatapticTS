...
export async function loadParentChunk(options = {}) {
  ...
  try {
    // Look up the sub-chunk to find its parent_chunk_id
    const subChunk = database
      .prepare(
        `
    ...
    if (subChunk.parent_chunk_id == null || Number(subChunk.depth) === 0) {
      throw new Error(
        `Chunk ${chunkId} is a top-level chunk (depth 0) with no parent.`,
      );
    }

    const parentId = Number(subChunk.parent_chunk_id);

    // Load the parent chunk with full v2 metadata
