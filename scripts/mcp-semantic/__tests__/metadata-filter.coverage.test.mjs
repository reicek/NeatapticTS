...
describe('metadata-filter — validateFilter success cases', () => {
  ...
  it('validates neq predicate', () => {
    expect(() =>
      validateFilter({ op: 'neq', field: 'arch_layer', value: 'network' }),
    ).not.toThrow();
  });

  it('validates eq with null value on numeric field', () => {
    expect(() =>
      validateFilter({ op: 'eq', field: 'depth', value: null }),
    ).not.toThrow();
  });

  it('validates eq with null value on enum field', () => {
    expect(() =>
      validateFilter({ op: 'eq', field: 'jsdoc_quality', value: null }),
    ).not.toThrow();
  });
  ...
  it('validates gt predicate on numeric field', () => {
    expect(() =>
      validateFilter({ op: 'gt', field: 'depth', value: 5 }),
    ).not.toThrow();
  });

  it('validates gte predicate', () => {
    expect(() =>
      validateFilter({ op: 'gte', field: 'depth', value: 5 }),
    ).not.toThrow();
  });

  it('validates lt predicate', () => {
    expect(() =>
      validateFilter({ op: 'lt', field: 'depth', value: 5 }),
    ).not.toThrow();
  });

  it('validates lte predicate', () => {
    expect(() =>
      validateFilter({ op: 'lte', field: 'depth', value: 5 }),
    ).not.toThrow();
  });

  it('validates like predicate', () => {
    expect(() =>
      validateFilter({ op: 'like', field: 'module_path', value: 'src/%' }),
    ).not.toThrow();
  });

  it('validates is_null predicate', () => {
    expect(() =>
      validateFilter({ op: 'is_null', field: 'arch_layer' }),
    ).not.toThrow();
  });

  it('validates is_not_null predicate', () => {
    expect(() =>
      validateFilter({ op: 'is_not_null', field: 'arch_layer' }),
    ).not.toThrow();
  });
  ...
  it('validates eq with enum value', () => {
    expect(() =>
      validateFilter({ op: 'eq', field: 'jsdoc_quality', value: 'good' }),
    ).not.toThrow();
  });

  it('validates eq with undefined value on enum field', () => {
    expect(() =>
      validateFilter({ op: 'eq', field: 'jsdoc_quality', value: undefined }),
    ).not.toThrow();
  });

  it('validates in with numeric values on numeric field', () => {
    expect(() =>
      validateFilter({ op: 'in', field: 'depth', values: [1, 2, 3] }),
    ).not.toThrow();
  });
});
...
describe('metadata-filter — validateFilter error cases', () => {
  ...
  it('throws on predicate count exceeded (and-specific limit)', () => {
    ...
    expect(() => validateFilter({ op: 'and', predicates })).toThrow(
      FilterError,
    );
  });
  ...
  it('throws on unknown operator', () => {
    expect(() => validateFilter({ op: 'bogus', field: 'arch_layer' })).toThrow(
      FilterError,
    );
  });
  ...
  it('throws on invalid field in eq', () => {
    expect(() =>
      validateFilter({ op: 'eq', field: 'bogus', value: 'x' }),
    ).toThrow(FilterError);
  });

  it('throws on non-numeric value for numeric field in eq', () => {
    expect(() =>
      validateFilter({ op: 'eq', field: 'depth', value: 'string' }),
    ).toThrow(FilterError);
  });

  it('throws on invalid enum value in eq', () => {
    expect(() =>
      validateFilter({ op: 'eq', field: 'jsdoc_quality', value: 'bogus' }),
    ).toThrow(FilterError);
  });

  it('throws on invalid field in in predicate', () => {
    expect(() =>
      validateFilter({ op: 'in', field: 'bogus', values: ['x'] }),
    ).toThrow(FilterError);
  });

  it('throws when in values is not an array', () => {
    expect(() =>
      validateFilter({ op: 'in', field: 'arch_layer', values: 'not-array' }),
    ).toThrow(FilterError);
  });
  ...
  it('throws when in values is too long', () => {
    ...
    expect(() =>
      validateFilter({ op: 'in', field: 'arch_layer', values }),
    ).toThrow(FilterError);
  });
  ...
  it('throws when in value is non-numeric for numeric field', () => {
    expect(() =>
      validateFilter({ op: 'in', field: 'depth', values: ['string'] }),
    ).toThrow(FilterError);
  });

  it('throws when in value is invalid enum', () => {
    expect(() =>
      validateFilter({ op: 'in', field: 'jsdoc_quality', values: ['bogus'] }),
    ).toThrow(FilterError);
  });
  ...
  it('throws on non-numeric range value', () => {
    expect(() =>
      validateFilter({ op: 'gt', field: 'depth', value: 'string' }),
    ).toThrow(FilterError);
  });

  it('throws on invalid field in like', () => {
    expect(() =>
      validateFilter({ op: 'like', field: 'bogus', value: 'x%' }),
    ).toThrow(FilterError);
  });

  it('throws on non-string like value', () => {
    expect(() =>
      validateFilter({ op: 'like', field: 'arch_layer', value: 123 }),
    ).toThrow(FilterError);
  });

  it('throws on invalid characters in like pattern', () => {
    expect(() =>
      validateFilter({ op: 'like', field: 'arch_layer', value: 'bad|pattern' }),
    ).toThrow(FilterError);
  });

  it('throws on invalid field in is_null', () => {
    expect(() => validateFilter({ op: 'is_null', field: 'bogus' })).toThrow(
      FilterError,
    );
  });

  it('throws on invalid field in is_not_null', () => {
    expect(() => validateFilter({ op: 'is_not_null', field: 'bogus' })).toThrow(
      FilterError,
    );
  });
  ...
  it('throws when and has fewer than 2 predicates', () => {
    expect(() =>
      validateFilter({
        op: 'and',
        predicates: [{ op: 'eq', field: 'arch_layer', value: 'x' }],
      }),
    ).toThrow(FilterError);
  });
  ...
  it('throws when not predicate is missing inner', () => {
    expect(() => validateFilter({ op: 'not', predicate: null })).toThrow(
      FilterError,
    );
  });

  it('throws when not predicate inner is not object', () => {
    expect(() => validateFilter({ op: 'not', predicate: 'string' })).toThrow(
      FilterError,
    );
  });
});
...
describe('metadata-filter — compileFilterToSql', () => {
  ...
  it('throws on unknown operator in compile', () => {
    expect(() =>
      compileFilterToSql({ op: 'bogus', field: 'arch_layer' }),
    ).toThrow(FilterError);
  });
...
describe('metadata-filter — compileFilterToSqlAliased', () => {
  it('compiles eq with aliases', () => {
    const { sql, params } = compileFilterToSqlAliased({
      op: 'eq',
      field: 'arch_layer',
      value: 'network',
    });
  ...
  it('throws on unknown operator in aliased compile', () => {
    expect(() =>
      compileFilterToSqlAliased({ op: 'bogus', field: 'arch_layer' }),
    ).toThrow(FilterError);
  });
