...
interface TypeScriptChunkContractReport {
  chunks: Array<{ doc_family: string; jsdoc_text: string; symbol_name: string }>;
}
...
describe('ts-chunker.mjs', () => {
  describe('red ts-source chunk contract', () => {
    it('emits one ts-source chunk per exported function with symbol name and JSDoc text', async () => {
      // Arrange
      const fixtureDirectory = await mkdtemp(path.join(tmpdir(), 'semantic-ts-chunker-red-'));
      ...
      const result = runModuleEvaluation<TypeScriptChunkContractReport>(`
        import { chunkTypeScriptSources } from './scripts/semantic-index/ts-chunker.mjs';
          ...
          chunks: [
            {
              ...
              jsdoc_text: 'Adds an evolved fitness score bonus while preserving deterministic ranking inputs.',
              symbol_name: 'addFitnessScore',
            },
            {
              doc_family: 'ts-source',
              jsdoc_text: 'Normalizes novelty into a bounded value for hybrid retrieval fixtures.',
