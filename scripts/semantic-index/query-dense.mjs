...
export async function queryDenseIndex(options = {}) {
  ...
  try {
    ...
    const candidateRowsByChunkId = mergeCandidateRows(
      bm25Results,
      denseCandidateRows,
    );
    const bm25ScoreByChunkId = new Map(
      bm25Results.map((result) => [result.chunk_id, -Number(result.score)]),
    );
    const rankedResults = rankHybridResults({
      ...
      candidates: [...candidateRowsByChunkId.values()].map((result) => ({
        ...
        embedding:
          result.embedding ??
          bm25EmbeddingByChunkId.get(result.chunk_id) ??
...
function loadEmbeddingsForChunks({ chunkIds, database, modelId }) {
  ...
  return new Map(
    rows.map((row) => [
      Number(row.chunk_id),
      decodeEmbeddingBlob(row.embedding),
    ]),
  );
}

function loadDenseCandidateRows({
  corpusDatabasePath,
  database,
  dimension,
  family,
  ...
  const allowedChunkIds = family
    ? loadFamilyChunkIds({ databasePath: corpusDatabasePath, family })
    : null;
  ...
  `,
    ...
    .toSorted(
      (leftRow, rightRow) => rightRow.cosine_score - leftRow.cosine_score,
    )
    .slice(0, limit);
  ...
  return rankedEmbeddingRows
    .map((embeddingRow) => {
      ...
      return chunkRow
        ? { ...chunkRow, embedding: embeddingRow.embedding }
        : null;
    })
    .filter(Boolean);
}
...
function mergeCandidateRows(bm25Results, denseCandidateRows) {
  const candidateRowsByChunkId = new Map(
    bm25Results.map((result) => [result.chunk_id, result]),
  );

  for (const denseCandidateRow of denseCandidateRows) {
    const existingCandidateRow = candidateRowsByChunkId.get(
      denseCandidateRow.chunk_id,
    );
    candidateRowsByChunkId.set(
      denseCandidateRow.chunk_id,
      existingCandidateRow
        ? { ...existingCandidateRow, embedding: denseCandidateRow.embedding }
        : denseCandidateRow,
    );
  }
...
function decodeEmbeddingBlob(blob) {
  const float32Length = Math.trunc(
    blob.byteLength / Float32Array.BYTES_PER_ELEMENT,
  );
  return Float32Array.from(
    new Float32Array(blob.buffer, blob.byteOffset, float32Length),
  );
}
