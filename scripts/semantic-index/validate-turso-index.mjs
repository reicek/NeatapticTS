 ...
 * (`data/semantic-index.sqlite` + `data/embeddings.sqlite`) read via
 ...
 *   `file:data/turso-replica.sqlite` for local development.
 ...
 *   Defaults to `data/semantic-index.sqlite`.
 * - `--source-embeddings <path>` — Override the source embeddings database path.
 *   Defaults to `data/embeddings.sqlite`.
 *
 * Exit codes:
 *
 ...
 * node scripts/semantic-index/validate-turso-index.mjs --json --validate-search
 *
 * // Turso Cloud
 * TURSO_DATABASE_URL=libsql://my-db.turso.io \
 * TURSO_AUTH_TOKEN=eyJ... \
 * node scripts/semantic-index/validate-turso-index.mjs --json --full
 *
 * // Programmatic usage
 ...
 */
const DEFAULT_SOURCE_CORPUS_PATH = 'data/semantic-index.sqlite';

/**
 * Default path to the source embeddings SQLite database (read-only comparison).
 */
const DEFAULT_SOURCE_EMBEDDINGS_PATH = 'data/embeddings.sqlite';
 ...
 *
 * The first seven tables are migrated from `data/semantic-index.sqlite`.
