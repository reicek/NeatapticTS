...
interface AggregatedMetric { mean:number; p50:number; p95:number; std:number }
...
function buildVariantDeltaTable(artifact: any): string {
  ...
  for (const size of ordered) {
    for (const metric of metrics) {
      const srcAgg: AggregatedMetric | undefined = agg.src?.[size]?.all?.[metric];
      const distAgg: AggregatedMetric | undefined = agg.dist?.[size]?.all?.[metric];
      const sMean = srcAgg?.mean; const dMean = distAgg?.mean;
      let dAbs: number|undefined; let dPct: number|undefined;
      if (typeof sMean==='number' && typeof dMean==='number') { dAbs = dMean - sMean; dPct = sMean===0?undefined:(dAbs/sMean)*100; }
