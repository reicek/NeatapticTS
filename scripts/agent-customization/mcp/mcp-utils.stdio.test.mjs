...
describe('runStdioMcpServer — Content-Length framing', () => {
  it('handles initialize request with Content-Length framing', async () => {
    ...
    mockStdin.write(contentLengthFrame({
      jsonrpc: '2.0',
      id: '1',
      method: 'initialize',
    }));
  ...
  it('writes error response for invalid JSON with Content-Length framing', async () => {
    ...
    mockStdin.write(`Content-Length: ${Buffer.byteLength(body)}\r\n\r\n${body}`);
    await wait();
    mockStdin.end();
    await promise;
    const response = parseFirstResponse(getStdout());
  ...
  it('handles multiple Content-Length messages in one chunk', async () => {
    const server = makeServer();
    const promise = runStdioMcpServer(server);
    const msg1 = contentLengthFrame({ jsonrpc: '2.0', id: '10', method: 'ping' });
    const msg2 = contentLengthFrame({ jsonrpc: '2.0', id: '11', method: 'ping' });
    ...
    const responses = output.split(/(?=Content-Length:)/).filter((s) => s.trim());
  ...
  it('handles partial Content-Length frame split across chunks', async () => {
    const server = makeServer();
    const promise = runStdioMcpServer(server);
    const frame = contentLengthFrame({ jsonrpc: '2.0', id: '12', method: 'ping' });
  ...
  it('handles leading line breaks before Content-Length frame', async () => {
    const server = makeServer();
    const promise = runStdioMcpServer(server);
    const frame = contentLengthFrame({ jsonrpc: '2.0', id: '13', method: 'ping' });
    mockStdin.write('\r\n\r\n' + frame);
    await wait();
    mockStdin.end();
    await promise;
...
describe('runStdioMcpServer — newline-delimited framing', () => {
  it('handles initialize request with newline framing', async () => {
    const server = makeServer();
    const promise = runStdioMcpServer(server);
    mockStdin.write(JSON.stringify({ jsonrpc: '2.0', id: '20', method: 'initialize' }) + '\n');
    await wait();
    mockStdin.end();
    await promise;
    const output = getStdout();
  ...
  it('handles CRLF line ending with newline framing', async () => {
    const server = makeServer();
    const promise = runStdioMcpServer(server);
    mockStdin.write(JSON.stringify({ jsonrpc: '2.0', id: '21', method: 'ping' }) + '\r\n');
    await wait();
    mockStdin.end();
    await promise;
    const output = getStdout();
  ...
  it('handles notification with newline framing — no response', async () => {
    const server = makeServer();
    const promise = runStdioMcpServer(server);
    mockStdin.write(JSON.stringify({ jsonrpc: '2.0', method: 'notifications/initialized' }) + '\n');
    await wait();
    mockStdin.end();
    await promise;
    expect(getStdout().trim()).toBe('');
  ...
  it('handles multiple newline-delimited messages', async () => {
    const server = makeServer();
    const promise = runStdioMcpServer(server);
    mockStdin.write(JSON.stringify({ jsonrpc: '2.0', id: '30', method: 'ping' }) + '\n');
    mockStdin.write(JSON.stringify({ jsonrpc: '2.0', id: '31', method: 'ping' }) + '\n');
    await wait();
    mockStdin.end();
    await promise;
  ...
  it('skips empty lines between messages', async () => {
    ...
    const promise = runStdioMcpServer(server);
    mockStdin.write('\n\n');
    mockStdin.write(JSON.stringify({ jsonrpc: '2.0', id: '32', method: 'ping' }) + '\n');
    mockStdin.write('\n\n');
    await wait();
    mockStdin.end();
    await promise;
...
describe('runStdioMcpServer — mixed framing', () => {
  it('handles Content-Length frame followed by newline-delimited frame', async () => {
    ...
    const clFrame = contentLengthFrame({ jsonrpc: '2.0', id: '50', method: 'ping' });
    // Newline-delimited frame after
    const nlFrame = JSON.stringify({ jsonrpc: '2.0', id: '51', method: 'ping' }) + '\n';
...
describe('runStdioMcpServer — non-Error in parser queued catch', () => {
  it('handles non-Error throw propagating through parser.push via stdout failure', async () => {
    ...
    stdoutSpy.mockImplementation(() => { throw 'write error'; });
    const promise = runStdioMcpServer(server);
