...
describe('neataptic-dispatch-mcp', () => {
  ...
  it('starts stdio server by default', async () => {
    mockMcpUtils.parseMcpCliArgs.mockReturnValue({ help: false, selfCheck: false });
  ...
  it('runs self-check when --self-check', async () => {
    mockMcpUtils.parseMcpCliArgs.mockReturnValue({ help: false, selfCheck: true });
    ...
    mockInventory.runCustomizationInventory.mockResolvedValue({
      agents: [
        { name: '01-planning', tier: '1', model: 'test', skills: [], agents: [], tools: [], userInvocable: true, path: '.github/agents/01-planning.agent.md' },
        { name: 'plan-scout', tier: '3', model: null, skills: [], agents: [], tools: [], userInvocable: false, path: '.github/agents/plan-scout.agent.md' },
      ],
    });
    mockDispatch.buildDispatchPacket.mockImplementation((req, agents) => {
      ...
      if (req.caller_tier > 1 && req.target_agent === '01-planning') {
        return { ok: false, dispatch_allowed: false, reason: 'Upward delegation not allowed' };
      }
      if (req.prompt && req.prompt.length > mockDispatch.PROMPT_LENGTH_MAX) {
        return { ok: false, dispatch_allowed: false, reason: 'Prompt too long', prompt_length: req.prompt.length, prompt_length_max: mockDispatch.PROMPT_LENGTH_MAX };
      }
  ...
  it('self-check records every issue branch when all responses are bad', async () => {
    ...
    mockMcpUtils.invokeServerRequest
      ...
      .mockResolvedValueOnce({ // list_dispatchable_agents -> 275, 297
        isError: true,
        structuredContent: { agents: [{ name: 'z' }, { name: 'a' }] },
      })
      .mockResolvedValueOnce({ // valid build_dispatch_packet -> 320, 330
        isError: true,
        structuredContent: { ok: false, dispatch_allowed: false },
      })
      .mockResolvedValueOnce({ // upward build_dispatch_packet -> 347, 361
        isError: true,
        structuredContent: { ok: true, dispatch_allowed: true },
      })
      .mockResolvedValueOnce({ // unknown build_dispatch_packet -> 378, 392
        isError: true,
        structuredContent: { ok: true, dispatch_allowed: true },
      })
      .mockResolvedValueOnce({ // get_dispatch_policy -> 406, 419, 431, 443
        isError: true,
        structuredContent: {},
      })
      .mockResolvedValueOnce({ // overlong prompt -> 472
        structuredContent: { ok: true, dispatch_allowed: true },
      });
    ...
    assert.ok(issues.length >= 16, `expected >=16 issues, got ${issues.length}`);
  });
  ...
  it('self-check reports issue when list_dispatchable_agents returns empty agents', async () => {
    ...
    mockMcpUtils.invokeServerRequest
      ...
      .mockResolvedValueOnce({ // tools/list ok (3 tools, correct names)
        tools: [
      ...
      .mockResolvedValueOnce({ // list_dispatchable_agents -> 285 (no agents array)
        isError: false,
        structuredContent: {},
      })
      .mockResolvedValueOnce({ // valid dispatch ok
        structuredContent: { ok: true, dispatch_allowed: true },
      })
      .mockResolvedValueOnce({ // upward dispatch ok
        structuredContent: { ok: false, dispatch_allowed: false },
      })
      .mockResolvedValueOnce({ // unknown dispatch ok
        structuredContent: { ok: false, dispatch_allowed: false },
      })
      .mockResolvedValueOnce({ // get_dispatch_policy ok
        structuredContent: { allowed_edges: [{ from: 0, to: 1 }], prompt_length_max: 500, prompt_length_max_trivial: 200 },
      })
      .mockResolvedValueOnce({ // overlong prompt ok
        structuredContent: { ok: true, dispatch_allowed: true },
      });
    ...
    const report = mockMcpUtils.createSelfCheckReport.mock.calls[0];
    const issues = report[1];
    assert.ok(issues.some((i) => /did not return a non-empty agent list/.test(i.message)));
  });
  ...
  it('list_dispatchable_agents handler returns sorted agents', async () => {
    ...
    mockInventory.runCustomizationInventory.mockResolvedValue({
      agents: [
        { name: 'z-agent', tier: '3', skills: ['s1'], agents: ['a1'], tools: ['t1'], userInvocable: false, path: 'z.md' },
        { name: 'a-agent', tier: '1', model: 'm', skills: [], agents: [], tools: [], userInvocable: true, path: 'a.md' },
      ],
    });
    ...
    const listTool = server.tools.find((t) => t.name === 'list_dispatchable_agents');
  ...
  it('build_dispatch_packet handler delegates to buildDispatchPacket', async () => {
    ...
    mockDispatch.buildDispatchPacket.mockReturnValue({ ok: true, dispatch_allowed: true });

    await importModule([]);

    const server = mockMcpUtils.createMcpServer.mock.calls[0][0];
    const buildTool = server.tools.find((t) => t.name === 'build_dispatch_packet');
    const result = await buildTool.handler({ target_agent: 'test', caller_tier: 1, prompt: 'hello' });
  ...
  it('get_dispatch_policy handler returns policy info', async () => {
    ...
    const server = mockMcpUtils.createMcpServer.mock.calls[0][0];
    const policyTool = server.tools.find((t) => t.name === 'get_dispatch_policy');
  ...
  it('self-check covers ?? {} branches when responses lack structuredContent', async () => {
    ...
    const report = mockMcpUtils.createSelfCheckReport.mock.calls[0];
    const issues = report[1];
    assert.ok(issues.length >= 10, `expected >=10 issues, got ${issues.length}`);
  });
});