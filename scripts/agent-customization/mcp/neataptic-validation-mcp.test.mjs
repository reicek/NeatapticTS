...
describe('neataptic-validation-mcp', () => {
  ...
  it('runs self-check when --self-check', async () => {
    ...
    mockMcpUtils.runShellFreeCommand.mockResolvedValue({
      exitCode: 0,
      stdout: 'ok',
      stderr: '',
    });
  ...
  it('get_active_validation_allowlist handler returns snapshot', async () => {
    ...
    const allowlistTool = server.tools.find(
      (t) => t.name === 'get_active_validation_allowlist',
    );
  ...
  it('run_allowlisted_validation handler executes allow-listed command', async () => {
    ...
    const server = mockMcpUtils.createMcpServer.mock.calls[0][0];
    const runTool = server.tools.find(
      (t) => t.name === 'run_allowlisted_validation',
    );
  ...
  it('uses session override when available', async () => {
    mockMcpUtils.parseMcpCliArgs.mockReturnValue({
      help: false,
      selfCheck: false,
      plan: 'plans/default.md',
    });
    mockMcpUtils.runStdioMcpServer.mockResolvedValue();
    mockFs.existsSync.mockReturnValue(true);
    const { readFile } = await import('node:fs/promises');
    readFile.mockResolvedValue(
      JSON.stringify({ plan_path: 'plans/override.md' }),
    );
    mockPlanUtils.loadActivePlanContext.mockResolvedValue({
      activeStep: { number: 1, validationCommands: [] },
    });
  ...
  it('self-check records issue branches when responses are bad', async () => {
    ...
    mockMcpUtils.tokenizeShellSafeCommand.mockReturnValue([
      'node',
      'npm',
      'test',
    ]); // does not throw
    mockMcpUtils.invokeServerRequest
      ...
      .mockResolvedValueOnce({
        // get_active_validation_allowlist -> 223, 233, 244
        isError: true,
        structuredContent: {
          validationCommands: ['npm test'],
      ...
      .mockResolvedValueOnce({
        // run_allowlisted_validation 'npm test' -> 283
        isError: true,
        structuredContent: { exitCode: 1 },
      })
      .mockResolvedValueOnce({
        // rejectedUnknownCommand -> 302 (not rejected)
        isError: false,
      });
  ...
  it('self-check reports issue when no executable validation commands exist', async () => {
    ...
    mockMcpUtils.invokeServerRequest
      ...
      .mockResolvedValueOnce({
        // rejectedUnknownCommand rejected -> no 302
        isError: true,
      });
    ...
    const issues = report[1];
    assert.ok(
      issues.some((i) => /could not find any non-recursive/.test(i.message)),
    );
  });
