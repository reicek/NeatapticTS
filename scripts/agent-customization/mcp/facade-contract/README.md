...
All files live under `scripts/agent-customization/mcp/facade-contract/` in this
research session. The production facade implementation reads the
`*-tool-snapshot.json` files from `scripts/agent-customization/mcp/` (per the
plan); those snapshots are already positioned in source.

| File                          | Purpose                                                                                                |
| ----------------------------- | ------------------------------------------------------------------------------------------------------ |
| `cortex-real-snapshot.json`   | JSON-RPC `initialize`, `tools/list`, and safe `tools/call` responses from the real Repo Cortex server. |
| `cortex-self-check.json`      | Expected self-check output for the `cortex` facade.                                                    |
| `devtools-real-snapshot.json` | JSON-RPC `initialize` and `tools/list` from the real `devtools` package.                               |
| `capture-cortex-snapshot.mjs` | Script that regenerates `cortex-real-snapshot.json` from the real Repo Cortex server.                  |
| `README.md`                   | This document.                                                                                         |
| `cortex-tool-snapshot.json`   | Single-router-tool snapshot the `cortex` facade exposes at session start. **Located in parent dir.**   |
| `devtools-tool-snapshot.json` | Single-router-tool snapshot the `devtools` facade exposes at session start. **Located in parent dir.** |
