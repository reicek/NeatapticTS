...
async function runWorkflowSelfCheck({ server, planPath }) {
  ...
  if (
    ...
    issues.push(
      selfCheckError(
        planPath,
        `Expected 2 workflow tools, found ${Array.isArray(toolListResult.tools) ? toolListResult.tools.length : 'none'}.`,
      ),
    );
  }
  ...
  if (
    workflowSnapshot.activePhase?.number !==
    activePlanContext.activePhase.number
  ) {
    issues.push(
      selfCheckError(
        planPath,
  ...
  if (
    workflowSnapshot.activeStep?.number !== activePlanContext.activeStep.number
  ) {
    issues.push(
      selfCheckError(
        planPath,
...
async function loadCustomizationInventory() {
  const commandResult = await runShellFreeCommand(INVENTORY_COMMAND, {
