/**
 * @module red-test-confirmation.gate.test
 * @description Coverage tests for red-test-confirmation.gate.mjs.
 */
...
describe('red-test-confirmation gate', () => {
  let originalLog;
  let logs;
  ...
  it('emits JSON contract with --json', async () => {
    await withArgv([process.execPath, 'dummy', '--json'], async () => {
      await jest.isolateModulesAsync(async () => {
        await import('./red-test-confirmation.gate.mjs');
      });
    });
    assert.equal(logs.length, 1);
    const parsed = JSON.parse(logs[0]);
    assert.equal(parsed.pass, true);
    assert.equal(parsed.evidence.gate, 'red-test-confirmation');
