...
import { compareDocsQualityRuns, REASON_CODES } from '../../semantic-index/docs-quality/docs-quality.compare.mjs';
...
export async function runDocsQualityMetricsGate() {
  ...
  const deterministicSchema = validateDocsQualityManifestV1(deterministicRun.manifest);
  ...
  const mcpServer = createRepoCortexMcpServer({ databasePath: './data/semantic-index.sqlite' });
  ...
  const parityMatch = hasMatchingManifestContractFields(baselineRun.manifest, mcpManifest);

  const invalidManifestCandidate = {
    ...baselineRun.manifest,
  };
  delete invalidManifestCandidate.scannerVersion;
  const invalidValidation = validateDocsQualityManifestV1(invalidManifestCandidate);

  const pass = baselineSchema.valid
    && deterministicSchema.valid
    && Number(baselineRun.manifest.metricVersion) === DOCS_QUALITY_METRIC_VERSION
    && String(baselineRun.manifest.scannerVersion) === DOCS_QUALITY_SCANNER_VERSION
    ...
    && baselineRun.manifest.normalizedEvidenceDigest === deterministicRun.manifest.normalizedEvidenceDigest
    && baselineRun.manifest.scopeDigest === deterministicRun.manifest.scopeDigest
    && mismatchCompare.accepted === false
    && mismatchCompare.reasonCode === REASON_CODES.THRESHOLD_MISMATCH
    && parityMatch
    && invalidValidation.valid === false
    && invalidValidation.errors.some((errorEntry) => errorEntry.field === 'scannerVersion');

  return {
    schema_version: 1,
    pass,
    gateType: 'mechanism-only',
    probeScope: 'docs-quality gate mechanism checks only: schema validity, deterministic ordering, comparator guards, CLI/MCP parity, and invalid contract rejection.',
    repoWideDebtCommand: 'npm run docs:quality:metrics',
    evidence: {
      schema_valid: baselineSchema.valid && deterministicSchema.valid,
      deterministic_ordering: baselineRun.manifest.normalizedEvidenceDigest === deterministicRun.manifest.normalizedEvidenceDigest,
      comparator_guards: mismatchCompare.accepted === false && mismatchCompare.reasonCode === REASON_CODES.THRESHOLD_MISMATCH,
...
async function main() {
  ...
  if (options.json) {
    ...
    console.log(report.pass ? 'PASS docs-quality-metrics gate' : 'FAIL docs-quality-metrics gate');
    if (!report.pass) {
...
function hasMatchingManifestContractFields(leftManifest, rightManifest) {
  ...
  return Number(leftManifest.metricVersion) === Number(rightManifest.metricVersion)
    && String(leftManifest.scannerVersion) === String(rightManifest.scannerVersion)
    && String(leftManifest.sourcePathsDigest) === String(rightManifest.sourcePathsDigest)
    && String(leftManifest.scopeDigest) === String(rightManifest.scopeDigest)
    && String(leftManifest.normalizedEvidenceDigest) === String(rightManifest.normalizedEvidenceDigest)
    && Number(leftManifest.thresholdConfig?.minJsdocWords) === Number(rightManifest.thresholdConfig?.minJsdocWords)
    && Number(leftManifest.thresholdConfig?.complexityThreshold) === Number(rightManifest.thresholdConfig?.complexityThreshold)
    && String(leftManifest.scopeConfig?.scopeType) === String(rightManifest.scopeConfig?.scopeType)
    && JSON.stringify(leftManifest.scopeConfig?.scopeValue ?? []) === JSON.stringify(rightManifest.scopeConfig?.scopeValue ?? []);
}