...
const DEFAULT_SNAPSHOT_PATH = path.join(repoRoot, 'docs', 'assets', 'semantic-snapshot.json');
const DEFAULT_WORKFLOW_PLAN_PATH = 'plans/mcp-active-binding.plans.md';
const WORKFLOW_MCP_PATH = path.join(repoRoot, 'scripts', 'agent-customization', 'mcp', 'neataptic-workflow-mcp.mjs');
...
export async function runCortexIndexGate(options = {}) {
  ...
  const snapshotMaxAgeMs = Number(options.snapshotMaxAgeMs ?? DEFAULT_SNAPSHOT_MAX_AGE_MS);
  const timeoutMs = Number(options.timeoutMs ?? DEFAULT_TIMEOUT_MS);
  const workflowPlanPath = options.workflowPlanPath ?? await resolveWorkflowPlanPath();

  const indexReport = await validateDatabase({ databasePath });
  const corpusMcpReport = await runCortexMcpSmoke({ databasePath });
  const workflowMcpReport = runWorkflowMcpSelfCheck({ timeoutMs, workflowPlanPath });
  const snapshotCurrency = await readSnapshotCurrency({ databasePath, snapshotPath, snapshotMaxAgeMs });

  const evidence = {
    index_documents: Number(indexReport.documents ?? snapshotCurrency.indexDocuments ?? 0),
  ...
  const pass = evidence.index_fresh
  ...
  return {
    ...
    pass,
    evidence,
    fixHint: pass ? null : resolveFixHint({ indexReport, snapshotCurrency, corpusMcpReport, workflowMcpReport }),
    owner: OWNER,
  };
}
...
function parseArgs(argv) {
  return {
    ...
    workflowPlanPath: argv.find((argument) => argument.startsWith('--plan='))?.slice('--plan='.length),
    snapshotMaxAgeMs: argv.find((argument) => argument.startsWith('--snapshot-max-age-ms='))?.slice('--snapshot-max-age-ms='.length),
  };
}
...
async function readSnapshotCurrency({ databasePath, snapshotPath, snapshotMaxAgeMs }) {
  ...
  try {
    const snapshotRow = database.prepare('SELECT COUNT(*) AS documents, MAX(indexed_at) AS indexed_at FROM documents').get();
    ...
    return {
      ...
      snapshotIndexedAt: indexedAtMs > 0 ? new Date(indexedAtMs).toISOString() : null,
    };
  } finally {
    database.close();
  }
...
function runWorkflowMcpSelfCheck({ timeoutMs, workflowPlanPath }) {
  const spawned = spawnSync(process.execPath, [WORKFLOW_MCP_PATH, `--plan=${workflowPlanPath}`, '--self-check', '--json'], {
    cwd: repoRoot,
    encoding: 'utf8',
    timeout: timeoutMs,
  });
...
async function resolveWorkflowPlanPath() {
  try {
    const configPayload = JSON.parse(await readFile(WORKFLOW_MCP_CONFIG_PATH, 'utf8'));
...
function resolveFixHint({ indexReport, snapshotCurrency, corpusMcpReport, workflowMcpReport }) {
  if (!indexReport.pass) {
...
async function main() {
  ...
  if (options.json) {
    ...
    console.log(report.pass ? 'PASS cortex-index gate' : 'FAIL cortex-index gate');
