...
export async function runCortexMcpSmoke(options = {}) {
  ...
  const searchResult = await invokeServerRequest(server, {
    ...
    params: {
      name: 'search_corpus',
      arguments: { query: 'NEAT activation', limit: 3 },
    },
  });
  ...
  if (searchResult.isError || results.length < 1) {
    return failReport(
      evidence,
      'search_corpus did not return results for NEAT activation.',
    );
  }
...
async function main() {
  ...
  const text = options.json
    ...
    : `${report.pass ? 'PASS' : 'FAIL'} cortex-mcp-smoke`;
  console.log(text);
}
