/**
 * @module log-completion-marker.gate.test
 * @description Coverage tests for log-completion-marker.gate.mjs.
 */
...
describe('log-completion-marker gate', () => {
  let originalLog;
  let logs;
  ...
  it('emits JSON contract with --json', async () => {
    await withArgv([process.execPath, 'dummy', '--json'], async () => {
      await jest.isolateModulesAsync(async () => {
        await import('./log-completion-marker.gate.mjs');
      });
    });
    assert.equal(logs.length, 1);
    const parsed = JSON.parse(logs[0]);
    assert.equal(parsed.pass, true);
    assert.equal(parsed.evidence.gate, 'log-completion-marker');
