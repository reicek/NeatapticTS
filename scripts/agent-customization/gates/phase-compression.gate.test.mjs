/**
 * @module phase-compression.gate.test
 * @description Coverage tests for phase-compression.gate.mjs.
 */
...
describe('phase-compression gate', () => {
  let originalLog;
  let logs;
  ...
  it('emits JSON contract with --json', async () => {
    await withArgv([process.execPath, 'dummy', '--json'], async () => {
      await jest.isolateModulesAsync(async () => {
        await import('./phase-compression.gate.mjs');
      });
    });
    assert.equal(logs.length, 1);
    const parsed = JSON.parse(logs[0]);
    assert.equal(parsed.pass, true);
    assert.equal(parsed.evidence.gate, 'phase-compression');
