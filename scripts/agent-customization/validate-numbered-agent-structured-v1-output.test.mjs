...
describe('validate-numbered-agent-structured-v1-output', () => {
  it('prints help and exits 0 when --help is passed', async () => {
    ...
    mockUtils.parseArgs.mockReturnValue({ help: true, json: false, contract: 'tier0' });
  ...
  it('validates a valid tier0 output', async () => {
    const origArgv = process.argv;
    process.argv = ['node', 'script.mjs', '--json', '--contract=tier0', '--input=test.md'];
    mockUtils.parseArgs.mockReturnValue({ help: false, json: true, contract: 'tier0', input: 'test.md' });
  ...
  it('validates a valid tier1 output', async () => {
    const origArgv = process.argv;
    process.argv = ['node', 'script.mjs', '--json', '--contract=tier1', '--input=test.md'];
    mockUtils.parseArgs.mockReturnValue({ help: false, json: true, contract: 'tier1', input: 'test.md' });
  ...
  it('supports contract aliases (tier-0, numbered, tier-1, coordinator)', async () => {
    for (const alias of ['tier-0', 'numbered', 'tier-1', 'coordinator']) {
      const origArgv = process.argv;
      process.argv = ['node', 'script.mjs', '--json', `--contract=${alias}`, '--input=test.md'];
      mockUtils.parseArgs.mockReturnValue({ help: false, json: true, contract: alias, input: 'test.md' });
      const isTier0 = ['tier-0', 'numbered'].includes(alias);
      mockFs.readFile.mockResolvedValue(isTier0 ? makeValidTier0Output() : makeValidTier1Output());

      await importModule();

      process.argv = origArgv;
      const report = mockUtils.writeReport.mock.calls[0][0];
      assert.strictEqual(report.ok, true, `alias ${alias} should produce ok report`);
      jest.resetModules();
      mockUtils = await import('./customization-utils.mjs');
      mockFs = await import('node:fs/promises');
      jest.clearAllMocks();
  ...
  it('flags error for unsupported contract', async () => {
    const origArgv = process.argv;
    process.argv = ['node', 'script.mjs', '--json', '--contract=invalid', '--input=test.md'];
    mockUtils.parseArgs.mockReturnValue({ help: false, json: true, contract: 'invalid', input: 'test.md' });
    mockFs.readFile.mockResolvedValue(makeValidTier0Output());

    await importModule();

    process.argv = origArgv;
    const contractIssue = mockUtils.issue.mock.calls.find(
      (c) => c[2].includes('Unsupported --contract'),
    );
  ...
  it('flags error when --input is missing', async () => {
    ...
    mockUtils.parseArgs.mockReturnValue({ help: false, json: true, contract: 'tier0' });

    await importModule();

    process.argv = origArgv;
    const inputIssue = mockUtils.issue.mock.calls.find(
      (c) => c[2].includes('requires --input'),
    );
  ...
  it('flags error when input file not found (ENOENT)', async () => {
    const origArgv = process.argv;
    process.argv = ['node', 'script.mjs', '--json', '--contract=tier0', '--input=missing.md'];
    mockUtils.parseArgs.mockReturnValue({ help: false, json: true, contract: 'tier0', input: 'missing.md' });
    ...
    process.argv = origArgv;
    const notFoundIssue = mockUtils.issue.mock.calls.find(
      (c) => c[2].includes('not found'),
    );
  ...
  it('flags error when output is not a structured-v1 fence', async () => {
    ...
    process.argv = origArgv;
    const fenceIssue = mockUtils.issue.mock.calls.find(
      (c) => c[2].includes('fenced'),
    );
  ...
  it('flags missing required fields', async () => {
    ...
    process.argv = origArgv;
    const missingIssues = mockUtils.issue.mock.calls.filter(
      (c) => c[2].includes('Missing required'),
    );
  ...
  it('flags unexpected fields', async () => {
    ...
    process.argv = origArgv;
    const unexpectedIssue = mockUtils.issue.mock.calls.find(
      (c) => c[2].includes('Unexpected'),
    );
  ...
  it('flags duplicate fields', async () => {
    ...
    process.argv = ['node', 'script.mjs', '--json', '--contract=tier0', '--input=test.md'];
    mockUtils.parseArgs.mockReturnValue({ help: false, json: true, contract: 'tier0', input: 'test.md' });
    const output = makeValidTier0Output().replace('SUMMARY: Done', 'SUMMARY: Done\nSUMMARY: Again');
    mockFs.readFile.mockResolvedValue(output);

    await importModule();

    process.argv = origArgv;
    const dupIssue = mockUtils.issue.mock.calls.find(
      (c) => c[2].includes('Duplicate'),
    );
    assert.ok(dupIssue);
  });
  ...
  it('flags invalid line syntax', async () => {
    ...
    process.argv = origArgv;
    const syntaxIssue = mockUtils.issue.mock.calls.find(
      (c) => c[2].includes('must match FIELD'),
    );
  ...
  it('flags list item without preceding field', async () => {
    ...
    process.argv = origArgv;
    const orphanIssue = mockUtils.issue.mock.calls.find(
      (c) => c[2].includes('List item must belong'),
    );
  ...
  it('flags list items in non-list-capable fields', async () => {
    ...
    process.argv = origArgv;
    const nonListIssue = mockUtils.issue.mock.calls.find(
      (c) => c[2].includes('does not accept list items'),
    );
  ...
  it('flags empty list items', async () => {
    ...
    const emptyListIssue = mockUtils.issue.mock.calls.find(
      (c) => c[2].includes('must match FIELD: value or list-item syntax'),
    );
  ...
  it('flags mixing inline values with list items', async () => {
    ...
    process.argv = origArgv;
    const mixIssue = mockUtils.issue.mock.calls.find(
      (c) => c[2].includes('cannot mix'),
    );
    assert.ok(mixIssue);
  });
  ...
  it('flags wrong OUTPUT_CONTRACT value', async () => {
    ...
    process.argv = ['node', 'script.mjs', '--json', '--contract=tier0', '--input=test.md'];
    mockUtils.parseArgs.mockReturnValue({ help: false, json: true, contract: 'tier0', input: 'test.md' });
    const output = makeValidTier0Output().replace('OUTPUT_CONTRACT: structured-v1', 'OUTPUT_CONTRACT: wrong');
    mockFs.readFile.mockResolvedValue(output);
    ...
    process.argv = origArgv;
    const contractIssue = mockUtils.issue.mock.calls.find(
      (c) => c[2].includes("OUTPUT_CONTRACT must equal 'structured-v1'"),
    );
    assert.ok(contractIssue);
  });
  ...
  it('flags invalid TASK_STATUS value', async () => {
    ...
    process.argv = ['node', 'script.mjs', '--json', '--contract=tier0', '--input=test.md'];
    mockUtils.parseArgs.mockReturnValue({ help: false, json: true, contract: 'tier0', input: 'test.md' });
    const output = makeValidTier0Output().replace('TASK_STATUS: SUCCESS', 'TASK_STATUS: INVALID');
    mockFs.readFile.mockResolvedValue(output);

    await importModule();

    process.argv = origArgv;
    const statusIssue = mockUtils.issue.mock.calls.find(
      (c) => c[2].includes('TASK_STATUS must be one of'),
    );
  ...
  it('flags wrong TIER value', async () => {
    ...
    process.argv = origArgv;
    const tierIssue = mockUtils.issue.mock.calls.find(
      (c) => c[2].includes("TIER must equal '0'"),
    );
    assert.ok(tierIssue);
  });
  ...
  it('flags empty scalar fields', async () => {
    ...
    process.argv = ['node', 'script.mjs', '--json', '--contract=tier0', '--input=test.md'];
    mockUtils.parseArgs.mockReturnValue({ help: false, json: true, contract: 'tier0', input: 'test.md' });
    const output = makeValidTier0Output().replace('ROLE: Orchestrator', 'ROLE: ');
    mockFs.readFile.mockResolvedValue(output);

    await importModule();

    process.argv = origArgv;
    const roleIssue = mockUtils.issue.mock.calls.find(
      (c) => c[2].includes('ROLE must be a non-empty'),
    );
    assert.ok(roleIssue);
  });
  ...
  it('flags wrong field order', async () => {
    ...
    process.argv = ['node', 'script.mjs', '--json', '--contract=tier0', '--input=test.md'];
    mockUtils.parseArgs.mockReturnValue({ help: false, json: true, contract: 'tier0', input: 'test.md' });
    const output = makeValidTier0Output().replace('TIER: 0\nROLE: Orchestrator', 'ROLE: Orchestrator\nTIER: 0');
    mockFs.readFile.mockResolvedValue(output);

    await importModule();

    process.argv = origArgv;
    const orderIssue = mockUtils.issue.mock.calls.find(
      (c) => c[2].includes('must appear in exact'),
    );
  ...
  it('defaults to tier0 contract when no --contract provided', async () => {
    ...
    mockUtils.parseArgs.mockReturnValue({ help: false, json: true, input: 'test.md' });
    mockFs.readFile.mockResolvedValue(makeValidTier0Output());
  ...
  it('flags invalid LEARNING_EVENT_NEEDED value', async () => {
    ...
    process.argv = ['node', 'script.mjs', '--json', '--contract=tier0', '--input=test.md'];
    mockUtils.parseArgs.mockReturnValue({ help: false, json: true, contract: 'tier0', input: 'test.md' });
    const output = makeValidTier0Output().replace('LEARNING_EVENT_NEEDED: false', 'LEARNING_EVENT_NEEDED: maybe');
    mockFs.readFile.mockResolvedValue(output);

    await importModule();

    process.argv = origArgv;
    const lenIssue = mockUtils.issue.mock.calls.find(
      (c) => c[2].includes('LEARNING_EVENT_NEEDED must be one of'),
    );
    assert.ok(lenIssue);
  });
  ...
  it('flags invalid PHASE_COMPLETE value for tier0', async () => {
    ...
    process.argv = ['node', 'script.mjs', '--json', '--contract=tier0', '--input=test.md'];
    mockUtils.parseArgs.mockReturnValue({ help: false, json: true, contract: 'tier0', input: 'test.md' });
    const output = makeValidTier0Output().replace('PHASE_COMPLETE: true', 'PHASE_COMPLETE: maybe');
    mockFs.readFile.mockResolvedValue(output);

    await importModule();

    process.argv = origArgv;
    const pcIssue = mockUtils.issue.mock.calls.find(
      (c) => c[2].includes('PHASE_COMPLETE must be one of'),
    );
    assert.ok(pcIssue);
  });
