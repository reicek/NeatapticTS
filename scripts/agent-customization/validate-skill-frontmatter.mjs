...
const issues = (
  await Promise.all(skills.map((skill) => validateSkill(skill, options)))
).flat();
const report = {
...
async function validateSkill(skill, { strict }) {
  ...
  if (!data.name)
    issues.push(issue('error', relativePath, 'Skill name is required.'));
  if (data.name && !/^[a-z0-9](?:[a-z0-9-]{0,62}[a-z0-9])?$/.test(data.name)) {
    issues.push(
      issue(
        'error',
        relativePath,
        'Skill name must be lowercase alphanumeric with hyphens and at most 64 characters.',
      ),
    );
  }
  if (data.name && data.name !== folderName) {
    issues.push(
      issue(
        'error',
        relativePath,
        `Skill name '${data.name}' must match folder '${folderName}'.`,
      ),
    );
  }
  if (!data.description)
    issues.push(issue('error', relativePath, 'Skill description is required.'));
  if (typeof data.description === 'string' && data.description.length > 1024) {
    issues.push(
      issue(
        'error',
        relativePath,
        'Skill description exceeds the Agent Skills 1024-character limit.',
      ),
    );
  }
  ...
  if (data.compatibility && String(data.compatibility).length > 500) {
    issues.push(
      issue(
        'error',
        relativePath,
        'Compatibility text exceeds the 500-character Agent Skills limit.',
      ),
    );
  }
  if (strict && !('user-invocable' in data)) {
    issues.push(
      issue(
        'warning',
        relativePath,
        'Strict mode prefers an explicit `user-invocable` decision.',
      ),
    );
  }
  if (strict && !data['argument-hint']) {
    issues.push(
      issue(
        'error',
        relativePath,
        'Strict mode requires an `argument-hint` for discoverable task shaping.',
      ),
    );
  }
  ...
  for (const localLink of extractMarkdownLinks(body)) {
    const target = path.posix.normalize(
      path.posix.join(path.posix.dirname(relativePath), localLink),
    );
    if (!(await fileExists(target))) {
      issues.push(
        issue(
          'error',
          relativePath,
          `Referenced local resource does not exist: ${localLink}`,
        ),
      );
    }
  }
