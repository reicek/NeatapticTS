...
if (options.help) {
  printUsage({
    ...
    usage:
      'node scripts/agent-customization/run-skill-trigger-evals.mjs [--json] [--input=scripts/agent-customization/evals/skill-trigger-evals.json]',
    options: [
      [
        '--input=<path>',
        'Trigger eval fixture with expected and optional observed trigger results.',
      ],
    ],
  });
  process.exit(0);
...
const inputPath =
  options.input ?? 'scripts/agent-customization/evals/skill-trigger-evals.json';
...
if (failures > 0)
  issues.push(issue('error', inputPath, `${failures} trigger evals failed.`));
if (pending > 0 && options.strict)
  issues.push(
    issue(
      'error',
      inputPath,
      `${pending} trigger evals are pending observed results.`,
    ),
  );
else if (pending > 0)
  issues.push(
...
function validateEvalCase(evalCase) {
  const issues = [];
  const allowedObservedKeys = new Set([
    'observedTriggered',
    'observedTarget',
    'observedNotes',
  ]);
    ...
    if (key.startsWith('observed') && !allowedObservedKeys.has(key)) {
      issues.push(
        issue(
          'error',
          inputPath,
          `${evalCase.id ?? 'UNKNOWN'} uses unsupported observed field '${key}'.`,
        ),
      );
    }
  }
  if (
    'observedTriggered' in evalCase &&
    typeof evalCase.observedTriggered !== 'boolean'
  ) {
    issues.push(
      issue(
        'error',
        inputPath,
        `${evalCase.id ?? 'UNKNOWN'} observedTriggered must be boolean when present.`,
      ),
    );
  }
  if (
    'observedTarget' in evalCase &&
    typeof evalCase.observedTarget !== 'string'
  ) {
    issues.push(
      issue(
        'error',
        inputPath,
        `${evalCase.id ?? 'UNKNOWN'} observedTarget must be a string when present.`,
      ),
    );
  }
  if (
    'observedNotes' in evalCase &&
    typeof evalCase.observedNotes !== 'string'
  ) {
    issues.push(
      issue(
        'error',
        inputPath,
        `${evalCase.id ?? 'UNKNOWN'} observedNotes must be a string when present.`,
      ),
    );
  }
  return issues;
