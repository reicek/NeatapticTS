 ...
 */
// @ts-expect-error dist build assumed present after npm run build
import {
...
void runBrowserBenchPayload({
  ...
  transferPayloadExporter: (network) =>
...
async function runAsyncBuildProbeComparisons() {
  ...
  for (const probeScenario of ASYNC_BUILD_PROBE_SCENARIOS) {
    asyncBuildComparisonRecords.push(
      await measureAsyncBuildScenario(probeScenario),
    );
  }
  ...
  try {
    ...
    const averageMacrotaskGapMs =
      macrotaskHeartbeatCount > 0
        ? elapsedMs / macrotaskHeartbeatCount
        : elapsedMs;
    ...
    return {
      ...
      microtaskYieldCount,
      asyncBuildsDelta:
        (asyncProbeNetwork._slabAsyncBuilds ?? 0) - asyncBuildsBefore,
