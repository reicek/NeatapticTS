 ...
 * `rag-index/data/turso-replica.sqlite` corpus DB, and validating that usable embeddings
...
const STEP_DEFINITIONS = Object.freeze([
  {
    args: [],
    name: 'download-model',
    scriptPath: path.join(repoRoot, 'rag-index', 'download-model.mjs'),
  },
  {
    args: [],
    name: 'embed-index',
    scriptPath: path.join(repoRoot, 'rag-index', 'embed-index.mjs'),
  },
  {
    args: ['--json'],
    name: 'validate-embeddings',
    scriptPath: path.join(repoRoot, 'rag-index', 'validate-embeddings.mjs'),
  },
  {
    args: [],
    name: 'download-reranker',
    scriptPath: path.join(repoRoot, 'rag-index', 'download-reranker.mjs'),
  },
  {
    args: ['--json'],
    name: 'validate-reranker',
    scriptPath: path.join(repoRoot, 'rag-index', 'reranker-readiness.mjs'),
  },
]);
...
async function main() {
  ...
  if (args.help) {
    printHelp({
      ...
      usage: 'node rag-index/prewarm-dense.mjs [--dry-run] [--json]',
      options: [
