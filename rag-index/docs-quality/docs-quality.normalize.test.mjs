...
describe('docs-quality.normalize.mjs', () => {
  describe('normalizeDocsQualityEvidence', () => {
    ...
    it('defaults issue and symbol to empty string when undefined', () => {
      const result = normalizeDocsQualityEvidence([
        { file: 'a.ts', issue: undefined, symbol: undefined },
      ]);
      expect(result).toHaveLength(0); // filtered because issue and symbol are empty
    });
    ...
    it('sorts by issue when severity, numericValue, file, and symbol are equal', () => {
      ...
      // "missing JSDoc" has severity rank 1, "weak JSDoc" has rank 3
      // So missing JSDoc should come first
    ...
    it('handles unknown issue type with severity rank 4', () => {
      ...
      // "weak JSDoc" has rank 3, "unknown issue" has rank 4
    ...
    it('handles incomplete JSDoc tags with severity rank 2', () => {
      ...
      // "missing JSDoc" has rank 1, "incomplete JSDoc tags" has rank 2
      expect(result[0].issue).toBe('missing JSDoc');
      expect(result[1].issue).toBe('incomplete JSDoc tags');
    });

    it('sorts by issue text when severity, numericValue, file, and symbol are all equal', () => {
      const result = normalizeDocsQualityEvidence([
        { file: 'a.ts', issue: 'custom B', symbol: 'f', numericValue: 5 },
        { file: 'a.ts', issue: 'custom A', symbol: 'f', numericValue: 5 },
      ]);
      // Both have unknown issue rank 4, same numericValue, same file, same symbol
      // Should sort by issue text: "custom A" before "custom B"
      expect(result[0].issue).toBe('custom A');
      expect(result[1].issue).toBe('custom B');
    });
  });
  ...
  describe('computeNormalizedEvidenceDigest', () => {
    ...
    it('sorts by file first in digest comparison', () => {
      const rows = [
        { file: 'b.ts', issue: 'x', symbol: 's', numericValue: 1 },
        { file: 'a.ts', issue: 'x', symbol: 's', numericValue: 1 },
      ];
      const d1 = computeNormalizedEvidenceDigest([...rows]);
      const d2 = computeNormalizedEvidenceDigest([...rows].reverse());
      expect(d1).toBe(d2);
    ...
    it('sorts by issue when file is equal in digest comparison', () => {
      const rows = [
        { file: 'a.ts', issue: 'B', symbol: 's', numericValue: 1 },
        { file: 'a.ts', issue: 'A', symbol: 's', numericValue: 1 },
      ];
      const d1 = computeNormalizedEvidenceDigest([...rows]);
      const d2 = computeNormalizedEvidenceDigest([...rows].reverse());
      expect(d1).toBe(d2);
    ...
    it('sorts by symbol when file and issue are equal in digest comparison', () => {
      const rows = [
        { file: 'a.ts', issue: 'x', symbol: 'z', numericValue: 1 },
        { file: 'a.ts', issue: 'x', symbol: 'a', numericValue: 1 },
      ];
      const d1 = computeNormalizedEvidenceDigest([...rows]);
      const d2 = computeNormalizedEvidenceDigest([...rows].reverse());
      expect(d1).toBe(d2);
    ...
    it('sorts by numericValue when file, issue, and symbol are equal in digest comparison', () => {
      const rows = [
        { file: 'a.ts', issue: 'x', symbol: 's', numericValue: 10 },
        { file: 'a.ts', issue: 'x', symbol: 's', numericValue: 5 },
      ];
      const d1 = computeNormalizedEvidenceDigest([...rows]);
      const d2 = computeNormalizedEvidenceDigest([...rows].reverse());
      expect(d1).toBe(d2);
