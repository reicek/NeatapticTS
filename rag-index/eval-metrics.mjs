 ...
 * the chunk heading path or symbol name. The heading needle falls back to
 * `symbol_name` when `heading_path` is absent so symbol-level queries still
 * resolve.
 *
...
export function matchesQueryExpectation(result, querySpec) {
  ...
  const headingHaystack = normalizeNeedle(
    [result.heading_path, result.symbol_name].filter(Boolean).join(' '),
  );
