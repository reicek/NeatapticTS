...
async function main() {
  ...
  if (args.help) {
    printHelp({
      ...
      usage:
        'node rag-index/build-browser-snapshot.mjs [--database path] [--output path] [--dry-run] [--json]',
      options: [
        '--database <path> Path to SQLite semantic index (default: rag-index/data/turso-replica.sqlite)',
        '--output <path>   Path to browser JSON snapshot (default: rag-index/snapshots/semantic-snapshot.json)',
