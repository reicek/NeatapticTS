...
export function _speciate(this: NeatLike & {
  ...
  for (const species of this._species) {
    ...
    for (const member of species.members as GenomeDetailed[]) previousMembers.add(member._id);
  ...
  for (const genome of this.population) {
    ...
    for (const species of this._species) {
      const compatibilityDistance = this._compatibilityDistance(genome, species.representative as GenomeDetailed);
  ...
  if (typeof options.compatibilityThreshold === 'number') {
    if (options.compatibilityThreshold < minThreshold) options.compatibilityThreshold = minThreshold;
    if (options.compatibilityThreshold > maxThreshold) options.compatibilityThreshold = maxThreshold;
  }

  this._species = this._species.filter((species) => species.members.length > 0);
  this._species.forEach((species) => (species.representative = species.members[0] as GenomeDetailed));

  const ageProtection = options.speciesAgeProtection ?? { grace: 3, oldPenalty: 0.5 };
  for (const species of this._species) {
    ...
    if (speciesAge >= (ageProtection.grace ?? 3) * 10) {
      ...
      if (penalty < 1) (species.members as GenomeDetailed[]).forEach((member) => { if (typeof member.score === 'number') member.score! *= penalty; });
    }
  }
  ...
  if (options.speciesAllocation?.extendedHistory) {
    const stats = this._species.map((species) => {
      ...
      const sizes = members.map((member) => ({ nodes: member.nodes.length, conns: member.connections.length, score: member.score ?? 0, ent: this._structuralEntropy(member) }));
      const average = (arr: number[]) => arr.length ? arr.reduce((a, b) => a + b, 0) / arr.length : 0;
      const meanNodes = average(sizes.map((x) => x.nodes));
      const meanConns = average(sizes.map((x) => x.conns));
      let innovationSum = 0; let innovationCount = 0; let maxInnovation = -Infinity; let minInnovation = Infinity; let enabledCount = 0; let disabledCount = 0;
      for (const member of members) for (const connection of member.connections as ConnectionLike[]) { const innovation = connection.innovation ?? this._fallbackInnov(connection); innovationSum += innovation; innovationCount++; if (innovation > maxInnovation) maxInnovation = innovation; if (innovation < minInnovation) minInnovation = innovation; if (connection.enabled === false) disabledCount++; else enabledCount++; }
      const meanInnovation = innovationCount ? innovationSum / innovationCount : 0;
      return { id: species.id, size: species.members.length, best: species.bestScore, meanNodes, meanConns, meanInnovation, innovationRange: isFinite(maxInnovation) && isFinite(minInnovation) && maxInnovation > minInnovation ? maxInnovation - minInnovation : 0, enabledRatio: enabledCount + disabledCount ? enabledCount / (enabledCount + disabledCount) : 0 } as Record<string, unknown>;
    });
    this._speciesHistory.push({ generation: this.generation, stats });
  } else {
    this._speciesHistory.push({ generation: this.generation, stats: this._species.map((species) => ({ id: species.id, size: species.members.length, best: species.bestScore })) });
  }

  if (this._speciesHistory.length > 200) this._speciesHistory.shift();
}

export function _applyFitnessSharing(this: NeatLike & { _species: SpeciesLike[]; _compatibilityDistance: (a: GenomeDetailed, b: GenomeDetailed) => number; }) {
  ...
  if (sigma > 0) {
    for (const s of this._species) {
      ...
      for (let i = 0; i < members.length; i++) {
        const mi = members[i]; if (typeof mi.score !== 'number') continue;
        let sum = 0;
        for (let j = 0; j < members.length; j++) { const mj = members[j]; const d = i === j ? 0 : this._compatibilityDistance(mi, mj); if (d < sigma) { const r = d / sigma; sum += 1 - r * r; } }
        if (sum <= 0) sum = 1; mi.score = mi.score / sum;
      }
    }
  } else {
    for (const s of this._species) { const members = s.members as GenomeDetailed[]; const size = members.length || 1; for (const m of members) if (typeof m.score === 'number') m.score = m.score / size; }
  }
}

export function _sortSpeciesMembers(this: NeatLike, sp: SpeciesLike) { (sp.members as GenomeDetailed[]).sort((a, b) => (b.score || 0) - (a.score || 0)); }

export function _updateSpeciesStagnation(this: NeatLike & { _species: SpeciesLike[]; generation: number; }) {
  const win = (this.options as any).stagnationGenerations ?? 15;
  for (const s of this._species) { _sortSpeciesMembers.call(this, s); const top = (s.members as GenomeDetailed[])[0]; if ((top?.score ?? -Infinity) > (s.bestScore ?? -Infinity)) { s.bestScore = top.score ?? -Infinity; s.lastImproved = this.generation; } }
