...
export interface GenomeWithScore {
  ...
  score?: number;
  /**
   * Additional genome metadata (implementation-defined).
   */
  [key: string]: unknown;
}
...
export interface NeatLikeWithSelection extends NeatLike {
  ...
  options: {
    ...
    selection?: {
      /**
       * Strategy name (e.g., POWER, FITNESS_PROPORTIONATE, TOURNAMENT).
       */
      name?: string;
      /**
    ...
    /**
     * Additional options (implementation-defined).
     */
    [key: string]: unknown;
  };
  /**
  ...
  _getRNG: () => () => number;
  /**
   * When true, tournament size overflows return a random member instead of throwing.
   */
