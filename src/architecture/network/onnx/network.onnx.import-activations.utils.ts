...
export function assignActivationFunctions(
  ...
  function collectOperationsByLayer(
    ...
    return sourceOnnx.graph.node.reduce<OnnxActivationLayerOperations>(
      (operationsByLayer, graphNode) => {
        // Step 1: Parse export naming metadata from node name.
        const parseResult = parseActivationNode(
          graphNode.name ?? EMPTY_NODE_NAME,
        );
        // Step 2: Keep only supported activation operations.
        const operation = asSupportedActivationOperation(graphNode.op_type);
        // Step 3: Skip non-activation or non-export nodes.
        if (!parseResult || !operation) return operationsByLayer;
        // Step 4: Append operation to its export-layer bucket.
  ...
  function applyHiddenLayerActivations(
    ...
    // Step 1: Build tiny traversal contexts with stable hidden offsets.
    const hiddenLayerTraversalContexts =
      buildHiddenLayerTraversalContexts(context);
    // Step 2: Exit early when there are no hidden layers to assign.
    if (hiddenLayerTraversalContexts.length === NO_HIDDEN_TRAVERSAL_CONTEXTS)
      return;
    // Step 3: Delegate traversal execution to a focused helper.
  ...
  function applyHiddenLayerTraversalActivation(
    ...
    // Step 1: Build contiguous neuron indices for the current layer.
    // Step 2: Apply activation assignment per neuron.
  ...
  function applyOutputLayerActivation(
    ...
    // Step 1: Build output traversal context from shared assignment state.
    ...
    // Step 2: Resolve output activation with layer-default fallback behavior.
    ...
    // Step 3: Assign one resolved activation to all output nodes.
