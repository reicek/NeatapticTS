...
type FusedRecurrentImportScope = {
  network: Network;
  onnx: OnnxModel;
...
export function reconstructFusedRecurrentLayers(
  ...
  const reconstructionScope: FusedRecurrentImportScope = {
    network,
    onnx,
    hiddenLayerSizes,
    layerFactory,
 ...
 * @returns Nothing.
 */
function reconstructOneFusedFamily(
  scope: FusedRecurrentImportScope,
  ...
  emittedLayerIndices.forEach((exportLayerIndex) =>
    reconstructOneFusedLayer(scope, {
      spec,
      exportLayerIndex,
...
function reconstructOneFusedLayer(
  ...
  if (
    !isValidHiddenLayerIndex(scope.hiddenLayerSizes, context.hiddenLayerIndex)
  ) {
    return;
  }
 ...
 */
function isValidHiddenLayerIndex(
  hiddenLayerSizes: number[],
...
function deriveLayerNeighborhood(
  ...
  const hiddenNodes = filterNodesByType(network, HIDDEN_NODE_TYPE);
 ...
 */
function createHiddenLayerRange(
  hiddenLayerSizes: number[],
  hiddenLayerIndex: number,
): {
  start: number;
...
function derivePreviousLayerNodes(
  ...
  if (hiddenLayerIndex === EMPTY_EDGE_COLLECTION_LENGTH) {
    return filterNodesByType(network, INPUT_NODE_TYPE);
  }
...
function deriveNextLayerNodes(
  ...
  if (isLastHiddenLayer) return filterNodesByType(network, OUTPUT_NODE_TYPE);
...
function replaceHiddenNodes(
  ...
  const inputNodes = filterNodesByType(network, INPUT_NODE_TYPE);
  const outputNodes = filterNodesByType(network, OUTPUT_NODE_TYPE);
