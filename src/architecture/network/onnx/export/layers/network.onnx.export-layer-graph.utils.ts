...
function emitNonConvolutionBranch(
  ...
  if (
    shouldEmitRecurrentBranch(createRecurrentDecisionContext(traversalContext))
  ) {
...
function tryEmitResidualAddBranch(
  ...
  const residualSourceOutputName =
    traversalContext.layerOutputNamesByLayerIndex.get(residualSourceLayerIndex);
 ...
 */
function emitPerNeuronBranch(traversalContext: LayerTraversalContext): string {
