...
interface SubNodeMutationConfig {
  keep_gates?: boolean;
}
...
function disconnectInboundConnections(
  ...
  for (const inboundConnection of node.connections.in.toReversed()) {
...
function disconnectOutboundConnections(
  ...
  for (const outboundConnection of node.connections.out.toReversed()) {
    preserveGaterForReassignment(
      outboundConnection,
      node,
      preservedGaters,
...
function ungateConnectionsGatedByNode(network: Network, node: Node): void {
  for (const gatedConnection of node.connections.gated.toReversed()) {
