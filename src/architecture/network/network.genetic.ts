...
export const crossOver = (
  ...
  const NetworkConstructor = require('../network').default;
  ...
  network1Props.nodes.forEach((n, i) => (n.index = i));
  network2Props.nodes.forEach((n, i) => (n.index = i));
  ...
  for (let i = 0; i < size; i++) {
    ...
    const node1 = i < n1Size ? network1Props.nodes[i] : undefined;
    /** Parent 2 node gene at current index (undefined if beyond parent size). */
    const node2 = i < n2Size ? network2Props.nodes[i] : undefined;
    ...
    if (chosen) {
      ...
      offspringProps.nodes.push(nn);
    }
  }
  // Reassign indices after constructing node list.
  offspringProps.nodes.forEach((n, i) => (n.index = i));
  ...
  const n1conns: Record<string, ConnectionGene> = {};
  /** Map from innovation ID -> connection gene extracted from parent 2 (includes self connections). */
  const n2conns: Record<string, ConnectionGene> = {};
  network1Props.connections.concat(network1Props.selfconns).forEach((c) => {
  ...
  const chosenConns: ConnectionGene[] = [];
  ...
  keys1.forEach((k) => {
    ...
    if (n2conns[k]) {
      ...
      if (c1.enabled === false || c2.enabled === false) {
        ...
        const rp =
          network1Props._reenableProb ?? network2Props._reenableProb ?? 0.25;
      ...
      if (c1.enabled === false) {
        ...
        const rp = network1Props._reenableProb ?? 0.25;
  ...
  chosenConns.forEach((cd) => {
    if (cd.from < nodeCount && cd.to < nodeCount) {
      const from = offspringProps.nodes[cd.from];
      const to = offspringProps.nodes[cd.to];
