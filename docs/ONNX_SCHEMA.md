...
interface OnnxNode {
  op_type: 'Gemm' | ActivationOp | 'Concat' | 'Add' | 'LSTM' | 'GRU' | 'Conv' | 'MaxPool' | 'AveragePool' | 'Flatten';
  ...
  attributes?: Array<{ name: string; type: string; f?: number; i?: number; ints?: number[] }>; // Conv/Pool include kernel_shape,strides,pads; Flatten axis
}
```
...
`PrevOut` is `'input'` for the first non-input layer, else `Layer_{L-1}`. For recurrent layers a *second* previous-state input is present: first recurrent layer uses `hidden_prev`, others `hidden_prev_l{k}`.
