...
Parameters:
- `this` - Bound network instance to archive.
...
Parameters:
- `compressedConnections` - Compressed connection block to read from.
- `decodedWeights` - Decoded float64 weight sequence.
- `disabledMask` - Optional disabled-run mask expanded over all connections.
- `connectionIndex` - Zero-based index of the connection to rebuild.
...
- `payloadBytes` - Compressed archive payload bytes.
- `compression` - Archive compression codec.
- `options` - Optional incremental decode progress callbacks.
...
Parameters:
- `isDisabled` - Whether the disabled run mask marks this connection as disabled.
- `enabledStates` - Optional compressed enabled-states vector.
...
Parameters:
- `encodedGaterIndex` - Raw gater index from the compressed block (may be the null sentinel).
...
Parameters:
- `values` - Optional array of values to index into.
- `index` - Zero-based index to retrieve.
...
### validateCompressedConnectionVectors
...
Parameters:
- `compressedConnections` - Compressed connection block to validate.
- `connectionCount` - Expected number of connections.
