...
Parameters:
- `network` - Network instance to update with the best genome.
- `neatInstance` - NEAT runtime with optional warning hook.
- `bestGenome` - Best evolved genome or undefined when none exists.
- `clearState` - Whether to clear network state after adoption.
