...
Parameters:
- `commandEncoder` - Active WebGPU command encoder.
- `networks` - Networks to evaluate as a batch.
- `states` - Per-network GPU state entries.
- `stagingBuffer` - Reusable mappable staging buffer.
- `outputCount` - Number of output nodes per network.
...
Parameters:
- `computePass` - Active WebGPU compute pass encoder.
- `bufferSet` - GPU buffer set for the network.
- `levelBindGroups` - Per-level bind groups.
- `levelWorkgroupCounts` - Per-level workgroup dispatch counts.

### ensureBatchGpuStates
...
- `networks` - Networks to evaluate as a batch.
- `states` - Per-network GPU state entries.
- `pipelines` - Per-network compiled compute pipelines.
- `iterationCount` - Number of forward passes to record.
...
- `inputCount` - Number of input nodes per network.
- `skipUpload` - When true, skip the upload entirely.

### validateBatchGpuEligibility
...
- `networks` - Networks to evaluate as a batch.
- `device` - WebGPU device used to run the forward kernel.
- `supportedActivations` - Set of supported activation indices.
...
```ts
buildEnvironmentInfo(
  options: { timestampQuerySupported?: boolean | undefined; gpuTimestampQueryNs?: number | null | undefined; gpuAdapterInfo?: Record<string, unknown> | null | undefined; gpuProbedLimits?: Record<string, number> | null | undefined; },
...
Parameters:
- `options` - Raw options with probed GPU adapter info and limits.
...
buildPerTierResults(
  ...
  tiers: number[],
  inputCount: number,
): { hiddenNodes: number; totalNodes: number; success: boolean; error: string | undefined; timings: { totalForwardPassMs: number; cpuPreparationMs: number; cpuTopoSortMs: number; cpuCSRBuildMs: number; cpuConnectionSlabBuildMs: number; cpuTopologyHashMs: number; bufferUploadMs: number; pipelineMs: number; bindGroupMs: number; dynamicBufferUploadMs: number; queueSubmissionMs: number; gpuCompletionWaitMs: number; outputReadbackMs: number; }; phases: ProfilingPhaseTiming[]; dominantBottleneck: string; overheadRatio: number; weakPoints: { name: "cpuPreparation" | "bufferUpload" | "pipeline" | "bindGroup" | "dynamicBufferUpload" | "queueSubmission" | "gpuCompletionWait" | "outputReadback"; impactPct: number; strategy: string; }[]; }[]
```

Build per-tier result entries from raw profiling results.

Parameters:
- `tierResults` - Raw profiling results from instrumented forward passes.
- `tiers` - Hidden-node tier values corresponding to each result.
- `inputCount` - Number of input nodes, used to compute total node counts.
...
### computeRankedWeakPoints

```ts
computeRankedWeakPoints(
  perTier: readonly { dominantBottleneck: string; }[],
): { name: string; tierCount: number; strategy: string; }[]
```

Compute ranked weak points by counting dominant-bottleneck occurrences across tiers.

Parameters:
- `perTier` - Per-tier summary entries.
...
Parameters:
- `field` - Navigator property to read.

Returns: The field value, or `'node'` when `navigator` is unavailable.

### resolveProfilingOptions

```ts
resolveProfilingOptions(
  options: { tiers?: number[] | undefined; inputCount?: number | undefined; outputCount?: number | undefined; referenceHardware?: { processor: string; memory: string; os: string; gpu_vendor: string; gpu_architecture: string; maxStorageBuffersPerShaderStage: number; } | undefined; browserVisibility?: string | undefined; },
): { tiers: number[]; inputCount: number; outputCount: number; referenceHardware: { processor: string; memory: string; os: string; gpu_vendor: string; gpu_architecture: string; maxStorageBuffersPerShaderStage: number; }; browserVisibility: string; }
```

Resolve optional profiling options to their default values.

Parameters:
- `options` - Raw options object with optional fields.
