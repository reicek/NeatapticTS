...
- **WebGPU inference** — opt-in GPU forward pass for eligible networks via `network.activate(input, { useGPU: true })`; automatic CPU fallback keeps classic NEAT behavior unchanged. See [docs/webgpu-inference.md](./docs/webgpu-inference.md) and the measured-performance notes in [docs/webgpu-performance-guide.md](./docs/webgpu-performance-guide.md).
