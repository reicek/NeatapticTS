...
// ---------------------------------------------------------------------------
// A2 Fix 8 — Additional high-priority allocation fixes
//
// - activateMlp pooled ping-pong activation buffers (currently allocates new
//   Float32Array per layer)
// - computeMovement flat Int32Array dx/dy pairs (currently clones direction
//   tuples)
// - updateEnemyController preallocated reusable Map/arrays
// - Pooled EnemyUpdateContext slots
// ---------------------------------------------------------------------------
...
describe('A2 Fix 8: Pooled activation buffers and flat movement arrays', () => {
  ...
  it('exports __testOnlyGetPooledContextSlots as a function from enemy-controller', async () => {
    const mod = (await import('./enemy-controller.ts')) as Record<
      string,
      unknown
    >;
    expect(typeof mod.__testOnlyGetPooledContextSlots).toBe('function');
  });

  it('exports __testOnlyGetReusableMapAllocCount as a function from enemy-controller', async () => {
    const mod = (await import('./enemy-controller.ts')) as Record<
      string,
      unknown
    >;
    expect(typeof mod.__testOnlyGetReusableMapAllocCount).toBe('function');
  });
});
