...
import { buildNeatensteinMap } from '../renderer/map';
...
import { NEATENSTEIN_AMMO_PICKUP_START_INDEX } from '../harness/neat-io-config';
...
// ---------------------------------------------------------------------------
// A2 Fix 3 — BFS distance map cache
//
// buildEnemyDistanceMap allocates a fresh Int32Array(14400) + DistanceMap
// every call.  The fix adds a module-level reusable Int32Array and caches
// the distance map per (map-seed, player-cell), only rebuilding when the
// player crosses a cell boundary.
// ---------------------------------------------------------------------------

describe('A2 Fix 3: BFS distance map cache', () => {
  it('exports getOrBuildCachedDistanceMap as a function', async () => {
    const mod = (await import('./enemy-navigation.ts')) as Record<
      string,
      unknown
    >;
    expect(typeof mod.getOrBuildCachedDistanceMap).toBe('function');
  });
  ...
  it('returns the same DistanceMap for the same player-cell and seed', async () => {
    ...
    // Same player-cell + same seed → same cached DistanceMap reference.
    expect(a).toBe(b);
