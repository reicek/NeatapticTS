...
describe('team-color cache key correctness', () => {
  it('returns the same cached reference for the same team color', async () => {
    const { resolveDecodedRobotSpriteFrameWithTeamColor } =
      await import('./sprites.atlas.utils');
    const first = resolveDecodedRobotSpriteFrameWithTeamColor(
      ENCODED_FRAME,
      [10, 20, 30],
    );
    const second = resolveDecodedRobotSpriteFrameWithTeamColor(
      ENCODED_FRAME,
      [10, 20, 30],
    );
  ...
  it('distinguishes all three channels independently', async () => {
    const { resolveDecodedRobotSpriteFrameWithTeamColor } =
      await import('./sprites.atlas.utils');
    const r = resolveDecodedRobotSpriteFrameWithTeamColor(
      ENCODED_FRAME,
      [1, 0, 0],
    );
    const g = resolveDecodedRobotSpriteFrameWithTeamColor(
      ENCODED_FRAME,
      [0, 1, 0],
    );
    const b = resolveDecodedRobotSpriteFrameWithTeamColor(
      ENCODED_FRAME,
      [0, 0, 1],
    );
    expect(r).not.toBe(g);
  ...
  it('distinguishes edge colors at byte boundaries', async () => {
    const { resolveDecodedRobotSpriteFrameWithTeamColor } =
      await import('./sprites.atlas.utils');
    const white = resolveDecodedRobotSpriteFrameWithTeamColor(
      ENCODED_FRAME,
      [255, 255, 255],
    );
    const black = resolveDecodedRobotSpriteFrameWithTeamColor(
      ENCODED_FRAME,
      [0, 0, 0],
    );
