...
const FLAPPY_INPUT_LABEL_GROUP_DEFINITIONS: readonly FlappyInputLabelGroupDefinition[] = [
  ...
  {
    label: FLAPPY_INPUT_GROUP_LABELS[2] ?? 'LOOKAHEAD',
    labelLines: ['LOOK', 'AHEAD'],
    tooltipHeading: 'Lookahead',
    tooltipBodyParagraphs: [
      'This group peeks one obstacle beyond the immediate pipe. It gives the policy a short planning horizon without needing a large explicit memory shelf.',
      'The distance row says how far away the following opening is, and the offset row says whether that later opening sits above or below the bird.',
      'Those hints reward smoother trajectories that stay ready for the next correction instead of solving only the current emergency.',
    ],
    nodeDescriptionDefinitions: [
      {
        ...
        tooltipBodyParagraphs: [
          'Lookahead distance says how far away the following opening is after the next pipe.',
          'That preview helps evolution favor paths that stay stable for consecutive obstacles instead of winning one pipe at a time.',
        ],
      },
      {
        ...
        tooltipBodyParagraphs: [
          "Lookahead offset measures whether the following gap sits above or below the bird's current line.",
          'It tells the controller whether today\'s correction should already prepare for a second move.',
        ],
      },
    ],
  ...
  {
    label: FLAPPY_INPUT_GROUP_LABELS[3] ?? 'CONTROL',
    labelLines: ['CONT', 'ROL'],
    tooltipHeading: 'Control Pressure',
    tooltipBodyParagraphs: [
      'This group summarizes action pressure rather than raw scene geometry alone. It condenses timing, safety, and remaining correction into decision-shaped hints.',
      'Time pressure captures urgency, gap clearance estimates breathing room, and needed climb estimates how much upward correction is still missing.',
      'Gap transition helps the network read whether the current gap hands off gently or sharply into the next one, which supports smoother flap rhythm.',
    ],
    nodeDescriptionDefinitions: [
      {
        ...
        tooltipBodyParagraphs: [
          'Time pressure compresses the current approach into an urgency signal.',
          'Higher pressure means the bird is running out of frames to correct cleanly, so the controller can distinguish setup moments from rescue moments.',
        ],
      },
      {
        ...
        tooltipBodyParagraphs: [
          'Gap clearance estimates how safely the bird currently sits inside the target corridor.',
          'It behaves like a comfort margin, letting the policy stay calm when there is room and react harder when safety is thin.',
        ],
      },
      {
        ...
        tooltipBodyParagraphs: [
          'Needed climb estimates how much upward correction is still required to reach the target path.',
          'That turns raw geometry into an action-shaped hint about whether another flap is still necessary.',
        ],
      },
      {
        ...
        tooltipBodyParagraphs: [
          'Gap transition summarizes how the current target hands off into the next one.',
          'It helps the controller prefer smooth, rhythmic flight lines over short-sighted one-pipe fixes.',
        ],
      },
    ],
    backgroundColor: FLAPPY_LIGHT_NEON_RAMP[6],
    orientation: 'vertical',
  },
] as const;
