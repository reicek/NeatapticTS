...
describe('processWorkerPlaybackStep', () => {
  it('downshifts future browser generations after a winner reaches the success pipe target', () => {
  ...
  it('keeps the existing browser population budget when the winner stays below the success pipe target', () => {
    const postWorkerMessage = jest.fn();
    const runtimeNetwork = createMockNetwork();
    const neatRuntime = {
      options: {
