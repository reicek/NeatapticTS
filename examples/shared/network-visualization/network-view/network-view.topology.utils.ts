...
import { NETWORK_LIGHT_NEON_RAMP } from './network-view.constants';
import type {
 ...
 * @param lightNeonRamp - Optional light neon ramp for hidden-column backgrounds.
...
export function resolveNetworkVisualizationTopologyPlan(
  ...
  const effectiveLightNeonRamp = lightNeonRamp ?? NETWORK_LIGHT_NEON_RAMP;
...
function resolveNarxTemporalTopologyPlan(
  ...
  const inputDelayColumns = inputDelayModule
    ? resolveModuleRoleColumns(inputDelayModule, nodeByGeneId, 0, lightNeonRamp)
    : { layers: [], annotations: [] };
  const outputDelayColumns = outputDelayModule
    ? resolveModuleRoleColumns(outputDelayModule, nodeByGeneId, 3, lightNeonRamp)
    : { layers: [], annotations: [] };
    ...
    return [
      {
        ...
        annotation: {
          ...
          backgroundColor:
            lightNeonRamp[(colorOffset + roleIndex) % lightNeonRamp.length] ??
            lightNeonRamp[0] ??
            '#7fe8ff',
  ...
  return {
    ...
    annotations: hiddenLayers.map((hiddenLayer, hiddenLayerIndex) => {
      ...
      return {
        ...
        tooltipBodyParagraphs: hiddenColumnTooltip.bodyParagraphs,
        backgroundColor:
          lightNeonRamp[(colorOffset + hiddenLayerIndex) % lightNeonRamp.length] ??
          lightNeonRamp[0] ??
          '#7fe8ff',
