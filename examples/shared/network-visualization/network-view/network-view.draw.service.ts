    ...
    return [
      {
        ...
        widthPx: NETWORK_INPUT_GROUP_LABEL_BAND_WIDTH_PX,
...
export function drawInputGroupLabelBands(
  ...
  const fontFamily = settings?.fontFamily ?? NETWORK_FONT_FAMILY;
  const statusText = settings?.palette?.statusText ?? NETWORK_NEON_PALETTE.statusText;

  context.save();
  context.fillStyle = NETWORK_INPUT_GROUP_LABEL_TEXT_COLOR;
  context.font = `${NETWORK_INPUT_GROUP_LABEL_FONT_WEIGHT} ${NETWORK_INPUT_GROUP_LABEL_FONT_SIZE_PX}px ${fontFamily}`;
    ...
    inputGroupLabelBandScene.labelLines.forEach((labelLine, labelLineIndex) => {
      ...
      context.fillText(
        ...
        labelCenterYPx -
          ...
          labelLineIndex * NETWORK_INPUT_GROUP_LABEL_LINE_HEIGHT_PX,
      );
    });
  });
...
export function drawInputNodeDescriptions(
  ...
  context.save();
  context.font = `${NETWORK_INPUT_DESCRIPTION_FONT_WEIGHT} ${NETWORK_INPUT_DESCRIPTION_FONT_SIZE_PX}px ${fontFamily}`;
  context.textAlign = 'center';
  context.textBaseline = 'middle';
  ...
  inputDescriptionScenes.forEach((inputDescriptionScene) => {
    ...
    context.fillStyle = NETWORK_INPUT_DESCRIPTION_TEXT_COLOR;
    ...
    inputDescriptionScene.labelLines.forEach((labelLine, labelLineIndex) => {
      context.fillText(
        ...
        descriptionCenterYPx -
          ...
          labelLineIndex * NETWORK_INPUT_DESCRIPTION_LINE_HEIGHT_PX,
      );
    });
  });
  ...
  return hiddenColumnAnnotations.flatMap((hiddenColumnAnnotation) => {
    ...
    const widthPx = Math.max(
      ...
      widestLineCharacterCount *
        NETWORK_HIDDEN_COLUMN_LABEL_CHARACTER_WIDTH_PX +
        NETWORK_HIDDEN_COLUMN_LABEL_TEXT_PADDING_PX * 2,
      maximumNodeRightPx - minimumNodeLeftPx,
    );
    const heightPx = Math.max(
      nodeDimensions.heightPx +
        NETWORK_HIDDEN_COLUMN_LABEL_TEXT_VERTICAL_PADDING_PX * 2,
...
export function drawHiddenColumnLabelScenes(
  ...
  context.save();
  context.font = `${NETWORK_HIDDEN_COLUMN_LABEL_FONT_WEIGHT} ${NETWORK_HIDDEN_COLUMN_LABEL_FONT_SIZE_PX}px ${fontFamily}`;
  context.textAlign = 'center';
  context.textBaseline = 'middle';
  ...
  hiddenColumnLabelScenes.forEach((hiddenColumnLabelScene) => {
    ...
    hiddenColumnLabelScene.labelLines.forEach((labelLine, labelLineIndex) => {
      context.fillStyle = NETWORK_HIDDEN_COLUMN_LABEL_TEXT_COLOR;
      context.fillText(
        labelLine,
        labelCenterXPx,
        labelCenterYPx -
          labelLineBlockHeightPx * 0.5 +
          labelLineIndex * NETWORK_HIDDEN_COLUMN_LABEL_LINE_HEIGHT_PX,
      );
    });
  });
 ...
 * @param leftXPx - Left edge x-coordinate.
 * @param topYPx - Top edge y-coordinate.
...
function resolveInputOverlayLayout(
  ...
  const descriptionRightXPx =
    inputLeftEdgeXPx - NETWORK_INPUT_DESCRIPTION_GAP_PX;
  const descriptionColumnLeftXPx = descriptionRightXPx - descriptionWidthPx;
  const labelBandRightXPx =
    descriptionColumnLeftXPx - NETWORK_INPUT_GROUP_LABEL_BAND_GAP_PX;

  return {
    descriptionColumnLeftXPx,
    descriptionRightXPx,
    labelBandLeftXPx:
      labelBandRightXPx - NETWORK_INPUT_GROUP_LABEL_BAND_WIDTH_PX,
  };
}
  ...
  const descriptionLayouts = inputDescriptions.map((inputDescription) => ({
    ...
    descriptionHeightPx: Math.max(
      ...
      inputDescription.labelLines.length *
        NETWORK_INPUT_DESCRIPTION_LINE_HEIGHT_PX +
        NETWORK_INPUT_DESCRIPTION_TEXT_VERTICAL_PADDING_PX * 2,
      nodeDimensions.heightPx +
        NETWORK_INPUT_DESCRIPTION_TEXT_VERTICAL_PADDING_PX * 2,
    ),
  }));
    ...
    (labelBand, labelBandIndex) => {
      ...
      const groupHeightPx = Math.max(
        ...
        groupedDescriptionContentHeightPx +
          NETWORK_INPUT_GROUP_PADDING_PX * 2,
      );
      ...
      if (labelBandIndex < labelBands.length - 1) {
        runningTopYPx += NETWORK_INPUT_GROUP_VERTICAL_GAP_PX;
      }
