...
export interface NetworkVisualizationResolvedFrame {
  canvasWidthPx: number;
  canvasHeightPx: number;
  /** Optional host-provided canvas background fill; falls back to the network default. */
  ...
  positionByNodeIndex: Map<number, PositionedNetworkNode>;
  runtimeConnections: VisualNetworkConnectionLike[];
  /** Optional host settings bag carried through to hover-only redraws. */
 ...
 * @param settings - Optional host settings bag (input label groups, canvas background, palette).
...
export function drawNetworkVisualization(
  ...
  const resolvedNetworkVisualizationFrame = resolveNetworkVisualizationFrame(
    ...
    inputSize,
    outputSize,
    settings?.inputLabelGroupDefinitions,
    settings,
  );
 ...
 * @param settings - Optional host settings bag; settings fields win over direct parameters.
...
export function resolveNetworkVisualizationFrame(
  ...
  // Step 1: Resolve effective input-label groups from settings with fallback.
  const effectiveInputLabelGroupDefinitions =
    settings?.inputLabelGroupDefinitions ?? inputLabelGroupDefinitions;

  // Step 2: Branch dense networks to the shared LOD abstraction when enabled.
  if (
    network &&
    settings?.lod?.enabled !== false &&
    shouldUseNetworkLOD(
      network,
      settings?.lod?.hiddenNodeThreshold ?? NETWORK_LOD_HIDDEN_NODE_THRESHOLD,
    )
  ) {
    return resolveNetworkLODFrame(context, network, inputSize, outputSize, {
      clusterCount: settings?.lod?.clusterCount,
      hoverMaxLocalNodes: settings?.lod?.hoverMaxLocalNodes,
      inputLabelGroupDefinitions: effectiveInputLabelGroupDefinitions,
      canvasBackground: settings?.canvasBackground,
    });
  }

  // Step 3: Resolve canvas, label, overlay, and padding state for this frame.
  const networkVisualizationScene = resolveNetworkVisualizationScene(
    context,
    network,
    inputSize,
    outputSize,
    effectiveInputLabelGroupDefinitions,
    settings,
  );

  // Step 4: Resolve node layout, node dimensions, and connection lookup state.
  ...
  // Step 5: Fold the reusable frame fields that hover-only redraws need later.
  return {
    canvasBackground: resolveNetworkCanvasBackground(settings),
    ...
    colorScales: applyNetworkVisualizationPaletteToColorScales(
...
export function drawResolvedNetworkVisualization(
  ...
  // Step 1: Delegate dense-network LOD frames to the dedicated LOD draw path.
  if (isNetworkLODResolvedFrame(resolvedNetworkVisualizationFrame)) {
    return drawNetworkLODFromFrame(
      context,
      resolvedNetworkVisualizationFrame,
  ...
  // Step 2: Paint the cached base canvas background before graph layers are drawn.
  paintNetworkVisualizationCanvasBase(
    context,
    resolvedNetworkVisualizationFrame,
  );

  // Step 3: Draw the graph layers and optional helper overlays from the cached frame.
  drawPositionedNetworkGraph(
    context,
    resolvedNetworkVisualizationFrame,
    hoverState,
    connectionLayerStyle,
    resolvedNetworkVisualizationFrame.settings,
  );

  // Step 4: Draw the legend after the graph so the overlay stays visually on top.
  drawNetworkColorLegend(
    context,
  ...
  // Step 5: Leave the canvas fill style at the panel base color so the
  // context rests in a known terminal state after the full draw pass.
  context.fillStyle =
    resolvedNetworkVisualizationFrame.canvasBackground ??
    NETWORK_UI_CANVAS_BACKGROUND;
...
function resolveNetworkVisualizationScene(
  ...
  const colorScales = resolveNetworkVisualizationColorScales(network, settings);
  const hideNetworkOverlays = shouldHideNetworkOverlays(
    context,
    settings?.overlayHiddenBreakpointPx,
  );
 ...
 */
function resolveNetworkCanvasBackground(
  settings?: NetworkVisualizationSettings,
): string {
  return settings?.canvasBackground ?? NETWORK_UI_CANVAS_BACKGROUND;
}
 ...
 *
 * @param colorScales - Color scales resolved from the active network.
 * @param palette - Optional host palette with the fields it wants to override.
 * @returns Color scales with palette overrides applied.
 */
function applyNetworkVisualizationPaletteToColorScales(
  colorScales: NetworkVisualizationColorScales,
  palette?: Partial<NetworkVisualizationPalette>,
): NetworkVisualizationColorScales {
  const currentRunText = palette?.currentRunText;
  if (currentRunText === undefined) {
    return colorScales;
  }
...
function paintNetworkVisualizationCanvasBase(
  ...
  networkVisualizationScene: Pick<
    NetworkVisualizationResolvedFrame,
    'canvasBackground' | 'canvasWidthPx' | 'canvasHeightPx'
  >,
): void {
  context.save();
  context.fillStyle =
    networkVisualizationScene.canvasBackground ?? NETWORK_UI_CANVAS_BACKGROUND;
  context.fillRect(
    0,
    0,
...
function resolveBaseGraphPaddingContext(
  ...
  return {
    graphLeftPaddingPx:
      NETWORK_GRAPH_LEFT_PADDING_PX + groupLabelBandReserveWidthPx,
    graphTopPaddingPx:
      NETWORK_GRAPH_TOP_PADDING_PX +
      resolveHiddenColumnLabelReserveHeightPx(network, hideNetworkOverlays),
    graphRightPaddingPx: NETWORK_GRAPH_RIGHT_PADDING_PX,
    graphBottomPaddingPx: NETWORK_GRAPH_BOTTOM_PADDING_PX,
  };
}
 ...
 *
 * @param context - Canvas 2D drawing context.
 * @param overlayHiddenBreakpointPx - Optional viewport breakpoint from settings.
...
function shouldHideNetworkOverlays(
  ...
  const effectiveBreakpointPx =
    overlayHiddenBreakpointPx ?? NETWORK_OVERLAY_HIDDEN_BREAKPOINT_PX;
  return context.canvas.width < effectiveBreakpointPx;
}
...
> {
  ...
  if (
    legendMidpointPx >=
    canvasWidthPx * NETWORK_LEGEND_RIGHT_SIDE_THRESHOLD_RATIO
  ) {
    return {
      graphLeftPaddingPx: graphPaddingContext.graphLeftPaddingPx,
...
function resolveNetworkNodeDimensionsFromTopologySummary(
  ...
  const availableNodeStackHeightPx = Math.max(
    NETWORK_MIN_DRAWABLE_SIZE_PX,
    drawableHeightPx - NETWORK_NODE_LAYOUT_PADDING_PX * 2,
  );
  ...
  return {
    widthPx: Math.max(
      ...
      Math.min(
        NETWORK_MAX_NODE_WIDTH_PX,
        nodeHeightPx * NETWORK_NODE_WIDTH_TO_HEIGHT_RATIO,
        drawableWidthPx /
          Math.max(
            NETWORK_NODE_WIDTH_LAYER_WIDTH_MIN_DENOMINATOR,
            networkTopologySummary.layerCount *
...
function resolveTopologyDrivenHeightPx(
  ...
  const minimumNodeSidePx =
    NETWORK_MIN_LABEL_HEIGHT_PX +
    NETWORK_MIN_NODE_INNER_PADDING_PX * 2;
...
function resolveRecommendedNetworkHeightPx(
  ...
  return Math.max(
    Math.ceil(
      topologyDrivenHeightPx * NETWORK_TOPOLOGY_HEIGHT_MULTIPLIER,
    ),
    NETWORK_BASELINE_HEIGHT_PX +
      nodeDensityHeightPx +
...
function formatArchitectureLabel(
  ...
  const architectureColumnsLabel = [
    ...
    hiddenLayersLabel,
    architectureOutputSize,
  ].join(NETWORK_ARCHITECTURE_COLUMN_SEPARATOR);
  ...
  return [
    ...
    .join(NETWORK_ARCHITECTURE_LINE_SEPARATOR);
}
...
function resolveHiddenLayersLabel(
  ...
  if (hiddenLayerSizes.length === 0) {
    return NETWORK_EMPTY_HIDDEN_LAYER_LABEL;
  }
  ...
  if (architectureSource === 'inferred') {
    return hiddenLayerSizes
      .map(
        (hiddenLayerSize) =>
          `${NETWORK_INFERRED_HIDDEN_LAYER_PREFIX}${hiddenLayerSize}`,
      )
      .join(NETWORK_HIDDEN_LAYER_SEPARATOR);
  }

  // Step 3: Join explicit hidden-layer sizes without inference prefixes.
  return hiddenLayerSizes.join(NETWORK_HIDDEN_LAYER_SEPARATOR);
}
