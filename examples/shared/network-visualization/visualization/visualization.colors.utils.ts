...
import type { ColorTier } from '../network-visualization.types';
 ...
 * @param settings - Optional host settings bag for ramps and palette overrides.
...
export function resolveNetworkVisualizationColorScales(
  ...
  const colorScaleOptions = {
    lightNeonRamp: settings?.lightNeonRamp ?? NETWORK_LIGHT_NEON_RAMP,
    regularNeonRamp: settings?.regularNeonRamp ?? NETWORK_REGULAR_NEON_RAMP,
    centerBlueRamp: settings?.centerBlueRamp ?? NETWORK_CENTER_BLUE_RAMP,
    aboveTierFallbackColor:
      settings?.palette?.currentRunText ?? NETWORK_NEON_PALETTE.currentRunText,
  };

  return {
    connectionScale: createDynamicColorScale(
      connectionValues,
      {
        minimumValue: -1,
        maximumValue: 1,
    ...
    biasScale: createDynamicColorScale(
      biasValues,
      {
        minimumValue: -1,
        maximumValue: 1,
...
function createDynamicColorScale(
  ...
  const resolvedLightNeonRamp =
    colorOptions?.lightNeonRamp ?? NETWORK_LIGHT_NEON_RAMP;
  const resolvedRegularNeonRamp =
    colorOptions?.regularNeonRamp ?? NETWORK_REGULAR_NEON_RAMP;
  const resolvedCenterBlueRamp =
    colorOptions?.centerBlueRamp ?? NETWORK_CENTER_BLUE_RAMP;
  const resolvedAboveTierFallbackColor =
    colorOptions?.aboveTierFallbackColor ??
    NETWORK_NEON_PALETTE.currentRunText;

  const dynamicTiers = resolveSignedDynamicTiers(
    minimumValue,
    maximumValue,
    resolvedLightNeonRamp,
    resolvedCenterBlueRamp,
    resolvedRegularNeonRamp,
  );
  const aboveTierColor =
    dynamicTiers.at(-1)?.color ?? resolvedAboveTierFallbackColor;
