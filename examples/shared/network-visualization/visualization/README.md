...
## visualization/visualization.types.ts

Visualization-specific color-scale contracts for network rendering.

Browser demos render connection weights and node biases with tiered neon
ramps so humans can quickly read sign and magnitude without parsing raw
...
## visualization/visualization.constants.ts

Geometry, alpha, and timing defaults for the shared network visualization
renderer.

These constants tune line weights, dashes, paddings, and hover transitions so
dense networks remain readable on a compact canvas.

Example:

```ts
const normalAlpha = NETWORK_DEFAULT_CONNECTION_ALPHA; // 0.3
const hoverAlpha = NETWORK_HIGHLIGHT_CONNECTION_ALPHA; // 0.9
```
...
```ts
const alpha = NETWORK_DEFAULT_CONNECTION_ALPHA; // 0.3
```
...
```ts
const alpha = NETWORK_HIGHLIGHT_CONNECTION_ALPHA; // 0.9
```
...
## visualization/visualization.draw.service.ts
...
drawWeightedConnectionsLayer(
  ...
  _settings: NetworkVisualizationSettings | undefined,
): void
```
...
- `_settings` - Optional host settings bag reserved for future connection theming.
...
- `nodeDimensions` - Shared node dimensions.
- `theme` - Resolved visualization theme.
...
## visualization/visualization.legend.utils.ts
...
## visualization/visualization.colors.utils.ts
...
always rendering a fixed generic scale. An optional
{@link NetworkVisualizationSettings} bag overrides the default ramps and
palette colors.

Parameters:
- `network` - Active network.
- `settings` - Optional host settings bag for ramps and palette overrides.
...
## visualization/visualization.topology.utils.ts
...
## visualization/visualization.errors.ts
