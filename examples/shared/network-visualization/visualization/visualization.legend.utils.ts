...
export function resolveNetworkLegendLayout(
  ...
  const compactLegend =
    context.canvas.width < NETWORK_LEGEND_COMPACT_WIDTH_THRESHOLD_PX ||
    context.canvas.height < NETWORK_LEGEND_COMPACT_HEIGHT_THRESHOLD_PX;
  const legendWidthPx = compactLegend
    ? NETWORK_LEGEND_COMPACT_WIDTH_PX
    : NETWORK_LEGEND_REGULAR_WIDTH_PX;
  const legendHeaderHeightPx = NETWORK_LEGEND_HEADER_HEIGHT_PX;
  ...
  const legendHeightPx =
    ...
    NETWORK_LEGEND_BOTTOM_PADDING_PX;
  const legendMarginPx = NETWORK_LEGEND_MARGIN_PX;
  const preferTopLeft =
    context.canvas.width < NETWORK_LEGEND_TOP_LEFT_THRESHOLD_PX;
  ...
  const minimumTopPx = Math.min(NETWORK_LEGEND_MIN_TOP_PX, maximumTopPx);
