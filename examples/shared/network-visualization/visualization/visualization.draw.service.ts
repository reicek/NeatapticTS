...
import { clamp } from '../network-visualization.math.utils';
import {
  NETWORK_FONT_FAMILY,
...
} from '../network-view/network-view.constants';
import type { NetworkVisualizationSettings } from '../network-view/network-view.types';
import type {
  ColorLegendRow,
...
import {
  NETWORK_DEFAULT_CONNECTION_ALPHA,
  NETWORK_DISABLED_CONNECTION_DASH_PATTERN,
  NETWORK_DIMMED_CONNECTION_ALPHA,
  NETWORK_HEADER_LINE_HEIGHT_PX,
  NETWORK_HEADER_PADDING_PX,
  NETWORK_HIGHLIGHT_CONNECTION_ALPHA,
  NETWORK_HIDDEN_NODE_STROKE_WIDTH_PX,
  NETWORK_HIDDEN_NODE_VERTICAL_PADDING_PX,
  NETWORK_HOVERED_NODE_STROKE_WIDTH_PX,
  NETWORK_LEGEND_ARCHITECTURE_GAP_PX,
  NETWORK_LEGEND_BIAS_LABEL_X_PX,
  NETWORK_LEGEND_BIAS_SWATCH_SIZE_PX,
  NETWORK_LEGEND_BIAS_SWATCH_X_PX,
  NETWORK_LEGEND_BIAS_SWATCH_Y_PX,
  NETWORK_LEGEND_BOX_PADDING_PX,
  NETWORK_LEGEND_CONNECTION_LABEL_X_PX,
  NETWORK_LEGEND_CONNECTION_SAMPLE_END_X_PX,
  NETWORK_LEGEND_CONNECTION_SAMPLE_Y_OFFSET_PX,
  NETWORK_LEGEND_HEADER_TOP_PADDING_PX,
  NETWORK_LEGEND_MIN_ARCHITECTURE_TOP_PX,
  NETWORK_MIN_RENDER_NODE_HEIGHT_PX,
  NETWORK_NEGATIVE_CONNECTION_DASH_PATTERN,
  NETWORK_OUTPUT_NODE_HEIGHT_REDUCTION_PX,
  NETWORK_OUTPUT_NODE_STROKE_WIDTH_PX,
...
const NETWORK_MULTILINE_LABEL_SEPARATOR = '\n';
const NETWORK_CANVAS_TEXT_ALIGN_LEFT: CanvasTextAlign = 'left';
const NETWORK_CANVAS_TEXT_ALIGN_CENTER: CanvasTextAlign = 'center';
const NETWORK_CANVAS_TEXT_BASELINE_TOP: CanvasTextBaseline = 'top';
const NETWORK_CANVAS_TEXT_BASELINE_ALPHABETIC: CanvasTextBaseline = 'alphabetic';
const NETWORK_CANVAS_LINE_CAP_BUTT: CanvasLineCap = 'butt';
const NETWORK_CANVAS_LINE_CAP_SQUARE: CanvasLineCap = 'square';
const NETWORK_TRANSPARENT_CANVAS_COLOR = 'transparent';
const NETWORK_OUTPUT_NODE_TYPE = 'output';
const NETWORK_LEGEND_TITLE = 'Legend';
const NETWORK_CONNECTION_SECTION_TITLE = 'Connection weight';
const NETWORK_BIAS_SECTION_TITLE = 'Node bias';
const NETWORK_CONNECTION_LEGEND_SYMBOL = 'w';
const NETWORK_BIAS_LEGEND_SYMBOL = 'b';
...
type ResolvedNetworkVisualizationTheme = {
  fontFamily: string;
  currentRunTextColor: string;
  headerTextColor: string;
  nodeLabelFillColor: string;
  hiddenNodeStrokeColor: string;
  outputNodeStrokeColor: string;
  outputNodeFillColor: string;
  legendBackgroundColor: string;
  legendStrokeColor: string;
  legendHeaderColor: string;
  legendConnectionTitleColor: string;
  legendBiasTitleColor: string;
  legendRowTextColor: string;
  overlayHiddenBreakpointPx: number;
};

function resolveNetworkVisualizationTheme(
  settings?: NetworkVisualizationSettings,
): ResolvedNetworkVisualizationTheme {
  const palette = settings?.palette ?? {};
  return {
    fontFamily: settings?.fontFamily ?? NETWORK_FONT_FAMILY,
    currentRunTextColor:
      palette.currentRunText ?? NETWORK_NEON_PALETTE.currentRunText,
    headerTextColor: settings?.theme?.headerText ?? NETWORK_HEADER_TEXT_COLOR,
    nodeLabelFillColor:
      settings?.theme?.nodeLabelFill ?? NETWORK_NODE_LABEL_FILL_COLOR,
    hiddenNodeStrokeColor:
      settings?.theme?.hiddenNodeStroke ?? NETWORK_HIDDEN_NODE_STROKE_COLOR,
    outputNodeStrokeColor:
      settings?.theme?.outputNodeStroke ?? NETWORK_OUTPUT_NODE_STROKE_COLOR,
    outputNodeFillColor:
      settings?.theme?.outputNodeFill ?? NETWORK_OUTPUT_NODE_FILL_COLOR,
    legendBackgroundColor:
      settings?.theme?.legendBackground ?? NETWORK_LEGEND_BACKGROUND,
    legendStrokeColor:
      settings?.theme?.legendStroke ??
      palette.statusText ??
      NETWORK_LEGEND_STROKE_COLOR,
    legendHeaderColor:
      settings?.theme?.legendHeader ?? NETWORK_LEGEND_HEADER_COLOR,
    legendConnectionTitleColor:
      settings?.theme?.legendConnectionTitle ??
      NETWORK_LEGEND_CONNECTION_TITLE_COLOR,
    legendBiasTitleColor:
      settings?.theme?.legendBiasTitle ?? NETWORK_LEGEND_BIAS_TITLE_COLOR,
    legendRowTextColor:
      settings?.theme?.legendRowText ?? NETWORK_LEGEND_ROW_TEXT_COLOR,
    overlayHiddenBreakpointPx:
      settings?.overlayHiddenBreakpointPx ?? NETWORK_OVERLAY_HIDDEN_BREAKPOINT_PX,
  };
}
...
const DEFAULT_WEIGHTED_CONNECTION_LAYER_STYLE: WeightedConnectionLayerStyle = {
  lineWidthPx: NETWORK_LEGEND_CONNECTION_LINE_WIDTH_PX,
  defaultConnectionOpacity: NETWORK_DEFAULT_CONNECTION_ALPHA,
  dimmedConnectionOpacity: NETWORK_DIMMED_CONNECTION_ALPHA,
  highlightConnectionOpacity: NETWORK_HIGHLIGHT_CONNECTION_ALPHA,
};
...
type LegendSceneContext = NetworkLegendLayout & {
  ...
  theme: ResolvedNetworkVisualizationTheme;
};
 ...
 *
 * The `settings` parameter is reserved for future connection theming and is
 * currently ignored; connection styling is still derived from the passed
 * `connectionScale` and `connectionLayerStyle`.
 *
 ...
 * @param _settings - Optional host settings bag reserved for future connection theming.
 * @returns Nothing.
 */
export function drawWeightedConnectionsLayer(
  ...
  _settings?: NetworkVisualizationSettings,
): void {
  void _settings;
  context.shadowBlur = 0;
  context.shadowColor = NETWORK_TRANSPARENT_CANVAS_COLOR;
  ...
  context.lineCap = NETWORK_CANVAS_LINE_CAP_BUTT;
}
 ...
 * @param animatedHoveredNodes - Optional animated hovered-node samples owned by the host canvas.
 * @param settings - Optional host settings bag.
 * @returns Nothing.
 */
export function drawBiasNodesLayer(
  ...
  const theme = resolveNetworkVisualizationTheme(settings);
...
export function drawNetworkVisualizationHeader(
  ...
  const headerLines = architectureLabel.split(NETWORK_MULTILINE_LABEL_SEPARATOR);
  const theme = resolveNetworkVisualizationTheme(settings);

  // Step 2: Paint the header block using the shared left-aligned text helper.
  drawLeftAlignedTextRows(context, {
    lines: headerLines,
    leftPx: NETWORK_HEADER_PADDING_PX,
    topPx: NETWORK_HEADER_PADDING_PX,
    lineHeightPx: NETWORK_HEADER_LINE_HEIGHT_PX,
    font: `${NETWORK_HEADER_FONT_SIZE_PX}px ${theme.fontFamily}`,
    fillStyle: theme.headerTextColor,
  });
}
...
export function drawNetworkColorLegend(
  ...
  if (shouldHideNetworkColorLegend(context, theme.overlayHiddenBreakpointPx)) {
    return;
  }
...
function resolveWeightedConnectionScene(
  ...
  return {
    ...
    lineCap:
      connectionWeight < 0
        ? NETWORK_CANVAS_LINE_CAP_SQUARE
        : NETWORK_CANVAS_LINE_CAP_BUTT,
  };
}
...
function resolveConnectionDashPattern(
  ...
  if (isNegativeConnection) {
    return NETWORK_NEGATIVE_CONNECTION_DASH_PATTERN;
  }

  if (!connectionEnabled) {
    return NETWORK_DISABLED_CONNECTION_DASH_PATTERN;
  }
...
function resolveBiasNodeScene(
  ...
  return {
    ...
    nodeLabelFillColor: theme.nodeLabelFillColor,
  };
}
...
function resolveBiasNodePaintStyle(
  ...
  return {
    ...
    nodeStrokeColor: isOutputNode
      ? theme.outputNodeStrokeColor
      : theme.hiddenNodeStrokeColor,
...
function resolveBiasNodeLabelMetrics(
  ...
  const computedLabelHeightPx = Math.floor(
    nodeDimensions.heightPx * NETWORK_NODE_LABEL_SIZE_RATIO,
  );
  ...
  const labelFont = `${NETWORK_NODE_LABEL_FONT_WEIGHT} ${labelHeightPx}px ${theme.fontFamily}`;

  // Step 2: Measure the label using the resolved font so baseline math stays stable.
  context.font = labelFont;
  context.textBaseline = NETWORK_CANVAS_TEXT_BASELINE_ALPHABETIC;
...
function resolveBiasNodeHeightPx(
  ...
  if (isOutputNode) {
    return Math.max(
      NETWORK_MIN_RENDER_NODE_HEIGHT_PX,
      nodeDimensions.heightPx - NETWORK_OUTPUT_NODE_HEIGHT_REDUCTION_PX,
    );
  }
...
function drawBiasNodeScene(
  ...
  context.fillStyle = biasNodeScene.nodeLabelFillColor;
  context.font = biasNodeScene.labelFont;
  context.textAlign = NETWORK_CANVAS_TEXT_ALIGN_CENTER;
  context.textBaseline = NETWORK_CANVAS_TEXT_BASELINE_ALPHABETIC;
  context.fillText(
...
function resolveLegendSceneContext(
  ...
  const architectureLines = architectureLabel.split(
    NETWORK_MULTILINE_LABEL_SEPARATOR,
  );
  const architectureLineCount = architectureLines.length;
  const architectureHeightPx =
    architectureLineCount * NETWORK_HEADER_LINE_HEIGHT_PX;
...
function drawLegendArchitectureLabel(
  ...
  drawLeftAlignedTextRows(context, {
    ...
    leftPx:
      legendSceneContext.legendLeftPx + NETWORK_LEGEND_BOX_PADDING_PX,
    topPx: legendSceneContext.architectureTextTopPx,
    lineHeightPx: NETWORK_HEADER_LINE_HEIGHT_PX,
    font: `${NETWORK_HEADER_FONT_SIZE_PX}px ${legendSceneContext.theme.fontFamily}`,
    fillStyle: legendSceneContext.theme.headerTextColor,
  });
}
...
function drawLegendFrame(
  ...
  context.fillStyle = legendSceneContext.theme.legendBackgroundColor;
  context.strokeStyle = legendSceneContext.theme.legendStrokeColor;
  context.lineWidth = 1;
  context.fillRect(
...
function drawLegendHeader(
  ...
  context.fillStyle = legendSceneContext.theme.legendHeaderColor;
  context.font = `${legendFontSizePx}px ${legendSceneContext.theme.fontFamily}`;
  context.textAlign = NETWORK_CANVAS_TEXT_ALIGN_LEFT;
  context.textBaseline = NETWORK_CANVAS_TEXT_BASELINE_TOP;
  context.fillText(
    NETWORK_LEGEND_TITLE,
    legendSceneContext.legendLeftPx + NETWORK_LEGEND_BOX_PADDING_PX,
    legendSceneContext.legendTopPx +
      NETWORK_LEGEND_HEADER_TOP_PADDING_PX,
  );
...
function drawLegendConnectionSection(
  ...
  context.fillStyle = legendSceneContext.theme.legendConnectionTitleColor;
  context.font = `${legendSceneContext.compactLegend ? NETWORK_LEGEND_COMPACT_FONT_SIZE_PX : NETWORK_LEGEND_REGULAR_FONT_SIZE_PX}px ${legendSceneContext.theme.fontFamily}`;
  context.textAlign = NETWORK_CANVAS_TEXT_ALIGN_LEFT;
  context.textBaseline = NETWORK_CANVAS_TEXT_BASELINE_TOP;
  context.fillText(
    NETWORK_CONNECTION_SECTION_TITLE,
    legendSceneContext.legendLeftPx + NETWORK_LEGEND_BOX_PADDING_PX,
...
function drawLegendConnectionRow(
  ...
  const sampleEndXPx =
    ...
    NETWORK_LEGEND_CONNECTION_SAMPLE_END_X_PX;
  const sampleYPx =
    connectionRowTopPx + NETWORK_LEGEND_CONNECTION_SAMPLE_Y_OFFSET_PX;
  ...
  context.fillStyle = legendSceneContext.theme.legendRowTextColor;
  context.fillText(
...
function drawLegendBiasSection(
  ...
  context.fillStyle = legendSceneContext.theme.legendBiasTitleColor;
  context.font = `${legendSceneContext.compactLegend ? NETWORK_LEGEND_COMPACT_FONT_SIZE_PX : NETWORK_LEGEND_REGULAR_FONT_SIZE_PX}px ${legendSceneContext.theme.fontFamily}`;
  context.textAlign = NETWORK_CANVAS_TEXT_ALIGN_LEFT;
  context.textBaseline = NETWORK_CANVAS_TEXT_BASELINE_TOP;
  context.fillText(
    NETWORK_BIAS_SECTION_TITLE,
    legendSceneContext.legendLeftPx + NETWORK_LEGEND_BOX_PADDING_PX,
    biasSectionTopPx,
  );
...
function drawLegendBiasRow(
  ...
  context.beginPath();
  context.rect(
    legendSceneContext.legendLeftPx + NETWORK_LEGEND_BIAS_SWATCH_X_PX,
    biasRowTopPx + NETWORK_LEGEND_BIAS_SWATCH_Y_PX,
    NETWORK_LEGEND_BIAS_SWATCH_SIZE_PX,
    NETWORK_LEGEND_BIAS_SWATCH_SIZE_PX,
  );
  context.fill();
  ...
  context.fillText(
    biasLegendRow.label,
    legendSceneContext.legendLeftPx + NETWORK_LEGEND_BIAS_LABEL_X_PX,
    biasRowTopPx - 1,
  );
}
