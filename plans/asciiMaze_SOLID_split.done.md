...
- Step 10 is now finalized around a narrow engine-facade support boundary: `evolutionEngine/evolutionEngine.constants.ts` now owns immutable warm-start, pretraining, and loop-tuning tables; `evolutionEngine/evolutionEngine.utils.ts` now owns the scratch-buffer node-index and hidden-to-output connection helpers consumed by loop-side inspection; and `evolutionEngine.ts` now binds those helpers while staying focused on public orchestration, mutable ring state, and stable compatibility exports.
- Step 11 is now finalized around engine-owned facade runtime state: `evolutionEngine/evolutionEngine.services.ts` now owns the shared engine singleton access, telemetry-toggle application, and mutable logits-ring runtime bookkeeping; `evolutionEngine/evolutionEngine.types.ts` now names that facade-runtime shape explicitly; and `evolutionEngine.ts` now delegates singleton and ring-state ownership to those services instead of carrying private mutable state fields.
- Step 12 is now finalized around typed evolution-loop context bundles: `evolutionEngine/evolutionEngine.types.ts` now owns named runtime, telemetry, and support contexts for the loop hot path; `evolutionEngine/evolutionLoop.ts` now consumes those contexts instead of repeating long positional runtime/helper lists across `runEvolutionLoop()` and `simulateAndPostprocess()`; and `evolutionEngine.ts` now assembles those contexts explicitly at the single orchestration call site.
...
- [DONE] Step 10: Extract immutable `EvolutionEngine` tuning tables and scratch-buffer inspection helpers into dedicated `evolutionEngine.constants.ts` and `evolutionEngine.utils.ts` modules so the public facade stays orchestration-first without changing imports.
- [DONE] Step 11: Move the remaining facade-owned engine singleton access, telemetry toggle application, and logits-ring runtime bookkeeping into `evolutionEngine.services.ts` so `evolutionEngine.ts` stays orchestration-first.
- [DONE] Step 12: Replace repeated `evolutionLoop` runtime/helper parameter bundles with named context objects in `evolutionEngine.types.ts` so the loop surface stays orchestration-first and less positional.
