...
```yaml
PlanUpdate:
  slice_id: 'P5-lean-roster-opt'
  phase: 5
  intent: 'Lean agent-roster optimization — fold ~44 thin wrapper agents into companion skills on their owning numbered agents; 2 merges; 1 demo removal; 4 gap-fill skills.'
  changed_files:
    ...
    - '.github/skills/benchmark-gate/SKILL.md'
    - '.github/skills/property-based-testing/SKILL.md'
    - 'scripts/agent-customization/tier-graph-utils.mjs'
    - '.github/agent-skill-routing-table.md'
  merges:
    - 'coverage-analyst (Tier 3) = coverage-scout + test-coverage-analyst + coverage-guard; skills [coverage-guard, coverage-tranche]'
    - 'agent-maintenance-coordinator (Tier 2) = helping-gap-resolution-coordinator + helping-agent-maintenance-coordinator; agents [learning-event-capturer]'
  new_skills:
    - 'security-review -> 04, 05'
    - 'dependency-audit -> 04, 06'
    - 'benchmark-gate -> 05'
    - 'property-based-testing -> 03'
  deletions:
    count: 46
    note: '45 fold-to-skill/merged-source agent files + 1 demo (flappy-architecture-polish) removed via Remove-Item -Force'
  reversals:
    - 'webgpu-scout removed (folded to webgpu skill on 04) — reverses earlier Phase 5 decision per critical audit'
  preflight:
    - 'npx prettier --write .github/agents/*.agent.md .github/skills/*/SKILL.md'
    - 'npm run agents:routing-table'
    - 'node scripts/agent-customization/validate-agent-frontmatter.mjs --json'
    - 'node scripts/agent-customization/validate-skill-frontmatter.mjs --json'
  tests_for_green:
  ...
  rollback:
    - 'Recreate 2 merged agents from source; restore 46 deleted agent files; revert roster edits; revert tier-graph-utils.mjs TIER_2/TIER_4 allow-lists'
  next: 'Lean roster delivered: 67 -> 23 agents, 67 skills. All validators green. Hand off to 05-green-testing for full validation pass.'
```

- agents:routing-table: PASS (exit 0, changed=true, agents=23, skills=67)
...
> **Phase 5 (P5-multi-tier-tree) SUPERSEDES the earlier P5-agent-skill-opt / P5-lean-roster-opt narrative above.** The prior "67 agents / webgpu-scout / 23-agent floor DONE" narrative was stale relative to the lean baseline. This block is the authoritative record of the multi-tier agent/subagent TREE built UP from the 23-agent floor, per the governing principle (a) specific POV lens / (b) isolated parallel context / (c) autonomous multi-step work — no thin skill-wrappers.
...
PlanUpdate:
  ...
  new_agents: 9
  new_skills: 0
  final_agent_count: 32
  final_skill_count: 67
  tier_distribution: { '1': 8, '2': 4, '3': 19, '4': 1 }
  new_agents_detail:
    - name: 'security-reviewer'
      tier: 3
      pov: 'Exploitable-vulnerability lens (distinct from 04 applying security-review skill inline)'
      justification: '(a) specific POV + (c) autonomous multi-step hunt'
      skills: ['security-review', 'implementation-standards']
      shared_by: ['04-implementing', '05-green-testing']
    - name: 'performance-reviewer'
      tier: 3
      pov: 'Performance-regression lens over traces and benchmarks'
      justification: '(a) specific POV'
      skills: ['performance-optimization', 'implementation-standards']
      shared_by: ['04-implementing', '05-green-testing']
    - name: 'api-contract-reviewer'
      tier: 3
      pov: 'Breaking-change / API-surface lens'
      justification: '(a) specific POV'
      skills: ['implementation-standards']
      shared_by: ['04-implementing', '06-documenting']
    - name: 'determinism-reviewer'
      tier: 3
      pov: 'RNG/seed/replay-stability lens'
      justification: '(a) specific POV'
      skills: ['reproducibility-contracts']
      shared_by: ['04-implementing', '05-green-testing']
    - name: 'slice-validator'
      tier: 3
      pov: 'Runs allow-listed validation commands, reports pass/fail with evidence'
      justification: '(b) isolated context + (c) autonomous command execution'
      skills: ['green-validation-gates', 'running-unit-tests']
      shared_by: ['03-red-testing', '05-green-testing']
    - name: 'license-reviewer'
      tier: 3
      pov: 'License/attribution lens on dependencies and source references'
      justification: '(a) specific POV'
      skills: ['license-attribution-audit']
      shared_by: ['02-researching', '06-documenting']
    - name: 'dependency-audit-reviewer'
      tier: 3
      pov: 'Supply-chain / dependency-vulnerability lens'
      justification: '(a) specific POV'
      skills: ['dependency-audit', 'security-review']
      shared_by: ['02-researching', '04-implementing']
    - name: 'benchmark-gate-reviewer'
      tier: 3
      pov: 'Benchmark/perf-gate regression lens'
      justification: '(a) specific POV'
      skills: ['benchmark-gate', 'performance-optimization']
      shared_by: ['02-researching', '05-green-testing']
  ...
  roster_changes:
    ...
    '01-planning': 'unchanged [plan-scout, agent-maintenance-coordinator]'
    '02-researching': '+ license-reviewer, dependency-audit-reviewer, benchmark-gate-reviewer'
    '03-red-testing': '+ slice-validator, property-based-test-writer, boundary-mapper (shared)'
    '04-implementing': '+ plan-scout (shared), performance-trace-specialist (shared), security-reviewer, performance-reviewer, api-contract-reviewer, determinism-reviewer, dependency-audit-reviewer'
    '05-green-testing': '+ plan-scout (shared), boundary-mapper (shared), slice-validator, security-reviewer, performance-reviewer, determinism-reviewer, benchmark-gate-reviewer'
    '06-documenting': '+ api-contract-reviewer, license-reviewer, browser-ui-specialist (shared), browser-memory-specialist (shared)'
    '07-logging': 'unchanged [plan-scout, learning-event-capturer, agent-maintenance-coordinator]'
  no_new_skills: 'All 9 new agents back EXISTING skills (verified against the 67 skill dirs). No tier-graph-utils.mjs change needed (new agents are Tier 3 = default fallback).'
  preflight:
    - 'npm run agents:routing-table'
    - 'node scripts/agent-customization/validate-agent-frontmatter.mjs --json'
    - 'node scripts/agent-customization/validate-skill-frontmatter.mjs --json'
    - 'npx prettier --check .github/agents/*.agent.md .github/skills/*/SKILL.md'
  tests_for_green:
    - 'npm run agents:routing-table:gate'
    - 'neataptic-gate-mcp:run_gate_check --gate=agent-graph'
    - 'neataptic-gate-mcp:run_gate_check --gate=tier-enforcement'
...
- agents:routing-table: PASS (exit 0, changed=true, agents=32, skills=67)
