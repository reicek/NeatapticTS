...
- [Spec-Kit_Assimilation.plans.md](Spec-Kit_Assimilation.plans.md): closed Spec-Kit Assimilation baseline for the `plans/constitution.md` five-principle governance authority, `constitution_check`/`spec-checklist` gate fields, clarification cap and traceability-ID discipline, `tasks-template.md`, `bug-triage` and `spec-checklist` skills, bug/extension/research catalogs, verbatim phrase alignment across 8 Tier-1 agents, 6 flow YAML files with constitutional gates, 11 RED scripts, and the expanded learning-event eventType schema.
...
- [rag-update.plans.md](rag-update.plans.md): closed RAG/index infrastructure reorganization baseline — consolidated Repo Cortex scripts, generated DB, model caches, freshness proofs, hook contexts, and snapshots under `rag-index/`; added the idempotent incremental `rag-index/update-rag.mjs` orchestrator; updated `package.json`, `jest.config.mjs`, `.gitignore`, MCP wiring, gates, skills, and agents. Phase 1 complete and archived.
...
- RAG/index reorganization reopen, rag-index consolidation reopen, semantic index move reopen, corpus DB relocation reopen, freshness-proof move reopen, update-rag.mjs reopen, rag:update script reopen: `plans/completed/rag-update.plans.md`
