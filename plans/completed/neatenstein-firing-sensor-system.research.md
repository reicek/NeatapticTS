...
const visibleEnemy = findNearestVisibleEnemy(
  state,
  flatMap,
  mapSize,
  VISION_RANGE,
...
export function findNearestVisibleEnemy(
  ...
  // LOS check
  if (!hasLineOfSight(flatMap, mapSize, p.position, nearest.position))
    return null;
...
nextState = {
  ...
  telemetry: withAimMissRate({
    ...
    shotsWasted:
      telemetryBeforeShot.shotsWasted + (hitType !== 'enemy' ? 1 : 0),
...
nextState = {
  ...
  telemetry: withAimMissRate({
    ...
    damageDealt:
      (nextState.telemetry?.damageDealt ?? 0) + NEATENSTEIN_BOLT_DAMAGE,
  }),
};
```
...
const fitnessFn = (network: Network): number => {
  ...
  for (let tick = 0; tick < NEATENSTEIN_FITNESS_MAX_EPISODE_TICKS; tick++) {
    ...
    const tickInput = networkOutputToTickInput(
      Array.isArray(raw)
        ? raw
        : new Array(NEATENSTEIN_MAIN_NEAT_OUTPUTS).fill(0),
    );
    state = gameTick(
      state,
      tickInput,
  ...
  const signal = extractCombatQualitySignal(
    finalState,
    finalState.telemetry ?? createDefaultTelemetry(),
  );
...
fitness =
  (survivalTicks / maxTicks) * 100 + // 0–100 scale
  damageDealt * 2 +
  kills * 5 -
  damageTaken * 1 -
  aimMissRate * 3 -
...
const killBonus =
  signal.kills * NEATENSTEIN_WEIGHT_KILLS * (1 + killEfficiency);
...
```typescript
fitness =
  survivalTicks * 1 + // survival floor
...
export function getFitnessWeights(generation: number): FitnessWeights {
  ...
  if (generation < 50)
    return { ...DEFAULT_WEIGHTS, aimMissRateWeight: 10, shotsWastedWeight: 1 };
  if (generation < 100)
    return {
      ...DEFAULT_WEIGHTS,
      aimMissRateWeight: 15,
      shotsWastedWeight: 2,
      killEfficiencyBonus: true,
    };
  return {
    ...DEFAULT_WEIGHTS,
    aimMissRateWeight: 20,
    shotsWastedWeight: 2,
    killEfficiencyBonus: true,
  };
}
