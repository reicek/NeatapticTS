...
## Standalone Cortex Orchestration Single Source of Truth Lane [DONE]

**Outcome:** use the Repo Cortex (Turso/libSQL RAG) as the single source of truth for orchestration context so agents query MCP directly instead of reading `.plans.md` and `.research` files, and execution sub-agents receive rich slice context automatically via a pre-execute hook. Five incremental phases: (A) enrich plan chunks with `slice_id` / `step_number` / `phase` / `status` metadata and expose those as `search_context` / `search_corpus` filters; (B) add a `get_slice_context` tool to `neataptic-workflow-mcp` that returns an assembled context window (step packet + source + boundary + test contracts) and route it through the lazy cortex facade plus all Tier-1 agent tool lists; (C) add a post-commit hook (and optional dev-mode file watcher) that detects changes to `plans/*.plans.md` and triggers a targeted `--files` re-index via `embed-index.mjs`, updates `cortex-index` gate fixHints, and logs re-index results so the index never goes stale during orchestration; (D) add a `pre_execute_hook` field to the step-packet convention so `04-implementing` specialists call `get_slice_context` before `read_file`; (E) migrate `02-researching`, `04-implementing`, and their specialists from `read_file`-first to Cortex-first retrieval, removing old direct-read patterns in the same slice (no dual-path, no deferred cleanup).

- Cortex Orchestration Single Source of Truth
- Plan: [completed/Cortex_Orchestration_Single_Source_of_Truth.plans.md](completed/Cortex_Orchestration_Single_Source_of_Truth.plans.md) [DONE]
- Final state: Phases A–E are [DONE] and compressed to `plans/completed/Cortex_Orchestration_Single_Source_of_Truth.logs.md`. Targeted RAG re-index, step-packet, plan-sync, log-completion-marker, and stale-wip-plans gates pass. Plan archived.
- Upstream dependencies: [completed/Repo_Cortex_Advanced_RAG_Architecture.plans.md](completed/Repo_Cortex_Advanced_RAG_Architecture.plans.md) [DONE] (provides `search_context`, metadata filtering, hybrid ranking, enriched `chunks` table), [completed/MCP_Lazy_Load_Facade.plans.md](completed/MCP_Lazy_Load_Facade.plans.md) [DONE] (provides the single `cortex` router facade this plan extends), soft dependency on [completed/Step_Packet_Goal_Redesign.plans.md](completed/Step_Packet_Goal_Redesign.plans.md) [DONE] (the `goal`-based step packet surface Phase D extends).

**Coordination rule:** this lane is confined to `rag-index/` (embed-index, migrations), `scripts/mcp-semantic/` (search-context, search-corpus, cortex-db), `scripts/agent-customization/` (workflow MCP, lazy cortex facade, step-packet gate), `.github/agents/` (Tier-1 and implementation/research specialist bodies), `.github/skills/` (`phase-handoff-workflow`, `research-methodology`, `repo-cortex-workflow`), and `plans/` tracker files. It does NOT change core NEAT algorithms in `src/neat/`, does NOT replace file-based plan authoring (humans still write `.plans.md`), does NOT remove `read_file` (it remains the documented degraded-Cortex fallback), and does NOT change the Turso cloud topology or DiskANN configuration. Treat [completed/Repo_Cortex_Advanced_RAG_Architecture.plans.md](completed/Repo_Cortex_Advanced_RAG_Architecture.plans.md) and [completed/MCP_Lazy_Load_Facade.plans.md](completed/MCP_Lazy_Load_Facade.plans.md) as the archived baselines.
...
## Standalone Meta-Workflow Lane — Orchestration Fix-Loop Hardening [DONE]
...
- Plan: [completed/orchestration-fixes.plans.md](completed/orchestration-fixes.plans.md) [DONE]
- Current internal state: all four phases (A–D) are [DONE] and compressed to `plans/completed/orchestration-fixes.logs.md`; plan archived. Phase A delivered policy/gate changes, Phase B delivered infrastructure fixes, Phase C delivered specialist workflow redesign with shared validation and RAG fix-packets plus the dispatch MCP prompt-length guard, Phase D delivered concurrency/RAG-exception documentation.
