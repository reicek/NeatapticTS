...
- [plans/Semantic_Knowledge_Dense_Prewarm.plans.md](Semantic_Knowledge_Dense_Prewarm.plans.md): Repo Cortex Layer 6 — operationalizes full embedding prewarm so `use_dense: true` becomes the MCP default from day 1; defines strict bootstrap-first contract, three readiness states (cold/model-only/warm), idempotent prewarm script, readiness probe, dense-readiness gate, and graceful MCP degradation when embeddings are cold. Depends on archived Layer 5 [DONE]. [PLANNED]
...
- dense prewarm, embedding prewarm, use_dense default, default-on dense, dense readiness, prewarm bootstrap, cold state degradation, dense-readiness gate, prewarm-dense, warm embeddings, dense default contract: `plans/Semantic_Knowledge_Dense_Prewarm.plans.md`
