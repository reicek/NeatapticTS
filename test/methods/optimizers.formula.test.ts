...
const data = Array.from({length:3}, (_,i)=>({ input:[i+1], output:[2*(i+1)] }));
...
function trainSteps(net: Network, opt: any, iterations: number, rate=0.01) {
  net.train(data, { iterations, rate, optimizer: opt, batchSize:1, error:0, cost: { fn:(t:number[],o:number[])=> ( (o[0]-t[0])**2 ), calculate:(t:number[],o:number[])=> ( (o[0]-t[0])**2 ) } });
}
...
describe('Optimizer formula characteristics', () => {
  describe('adamax infinity norm vs adam second moment', () => {
    ...
    it('maintains opt_u (infinity norm) different from sqrt(v)', () => { expect(connA.opt_u).not.toBeUndefined(); });
  });
  ...
  describe('nadam nesterov lookahead produces larger early step than adam', () => {
    ...
    it('has different first step magnitude from adam', () => { expect(Math.abs(deltaN - deltaA)).toBeGreaterThan(0); });
  });
  ...
  describe('radam early unrectified vs later rectified variance', () => {
    ...
    it('late step magnitude differs from very early step', () => { expect(Math.abs(lateDelta - earlyDelta)).toBeGreaterThan(0); });
  });
  ...
  describe('adabelief variance differs from adam given same gradients', () => {
    ...
    it('maintains distinct second moment estimate', () => { expect(cB.opt_v).not.toBe(cA.opt_v); });
  });
  ...
  describe('lookahead defaults', () => {
    ...
    it('creates shadow weight with default params', () => { expect(conn._la_shadowWeight).toBeDefined(); });
  });
});
