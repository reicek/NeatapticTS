...
import { installResponsiveViewportSizing } from './host.resize.service';
import {
...
export function createCanvasHostInternal(
  ...
  // Step 2: Create top-level layout containers.
  ...
  // Step 3: Create the header canvas used by the title renderer.
  ...
  // Step 4: Create the main simulation canvas.
  ...
  // Step 5: Create the stats and network split-pane container.
  ...
  // Step 7: Create the network visualization host and canvas.
  ...
  // Step 8: Track the last rendered network payload for resize redraws.
  ...
  const renderNetworkArchitecture: NetworkVisualizationHandle['renderNetworkArchitecture'] =
    (network, inputSize, outputSize) => {
      // Step 8.1: Resolve preferred height, then render current payload.
  ...
  // Step 9: Mount the host DOM tree in final order.
  ...
  const drawHeaderFrame = (): void => {
    // Step 9.1: Sync backing resolution to display size.
    ...
    // Step 9.2: Render the title box glyph frame.
  ...
  // Step 11: Render the initial placeholder visualization and header.
  renderNetworkArchitecture(
    undefined,
    FLAPPY_NETWORK_INPUT_SIZE,
    FLAPPY_NETWORK_OUTPUT_SIZE,
