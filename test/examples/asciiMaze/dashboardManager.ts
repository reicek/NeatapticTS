...
export class DashboardManager implements IDashboardManager {
   ...
   */
  update(maze: string[], result: any, network: INetwork, generation: number): void { // Changed Network to INetwork
  ...
  redraw(currentMaze: string[]): void {
    ...
    this.logFunction(`${colors.blueCore}╔${NetworkVisualization.pad('═', 148, '═')}╗${colors.reset}`);
    this.logFunction(`${colors.blueCore}╚${NetworkVisualization.pad('╦════════════╦', 148, '═')}╝${colors.reset}`);
    this.logFunction(`${colors.blueCore}${NetworkVisualization.pad(`║ ${colors.neonYellow}ASCII maze${colors.blueCore} ║`, 150, ' ')}${colors.reset}`);
    this.logFunction(`${colors.blueCore}╔${NetworkVisualization.pad('╩════════════╩', 148, '═')}╗${colors.reset}`);
    
    // Print current best for active maze
    if (this.currentBest) {
      this.logFunction(`${colors.blueCore}╠${NetworkVisualization.pad('══════════════════════', 148, '═')}╣${colors.reset}`);
      this.logFunction(`${colors.blueCore}║${NetworkVisualization.pad(`${colors.orangeNeon}EVOLVING (GEN ${this.currentBest.generation})`, 148, ' ')}${colors.blueCore}║${colors.reset}`);
      this.logFunction(`${colors.blueCore}╠${NetworkVisualization.pad('══════════════════════', 148, '═')}╣${colors.reset}`);
      this.logFunction(`${colors.blueCore}║${NetworkVisualization.pad(' ', 148, ' ')}║${colors.reset}`);
      
      // Visualize the network
      this.logFunction(`${colors.blueCore}║${NetworkVisualization.pad(' ', 148, ' ')}║${colors.reset}`);
      this.logFunction(NetworkVisualization.visualizeNetworkSummary(this.currentBest.network));
      this.logFunction(`${colors.blueCore}║${NetworkVisualization.pad(' ', 148, ' ')}║${colors.reset}`);
      ...
      const currentMazeVisualization = MazeVisualization.visualizeMaze(currentMaze, lastPos, this.currentBest.result.path);
      const currentMazeLines = Array.isArray(currentMazeVisualization) ? currentMazeVisualization : currentMazeVisualization.split('\n');
      const centeredCurrentMaze = currentMazeLines.map(line => `${colors.blueCore}║${NetworkVisualization.pad(line, 148, ' ')}${colors.blueCore}║`).join('\n');
      this.logFunction(`${colors.blueCore}║${NetworkVisualization.pad(' ', 148, ' ')}║${colors.reset}`);
      ...
      this.logFunction(`${colors.blueCore}║${NetworkVisualization.pad(' ', 148, ' ')}║${colors.reset}`);
      MazeVisualization.printMazeStats(this.currentBest, currentMaze, this.logFunction);
      this.logFunction(`${colors.blueCore}║${NetworkVisualization.pad(' ', 148, ' ')}║${colors.reset}`);
      
      // Show progress bar for agent's progress to exit
      this.logFunction(`${colors.blueCore}║${NetworkVisualization.pad(' ', 148, ' ')}║${colors.reset}`);
      this.logFunction(`${colors.blueCore}║ ${colors.neonSilver}Progress to exit: ${MazeVisualization.displayProgressBar(this.currentBest.result.progress)}`);
      this.logFunction(`${colors.blueCore}║${NetworkVisualization.pad(' ', 148, ' ')}║${colors.reset}`);
    }
    ...
    if (this.solvedMazes.length > 0) {
      this.logFunction(`${colors.blueCore}╠${NetworkVisualization.pad('╦══════════════╦', 148, '═')}╣${colors.reset}`);
      this.logFunction(`${colors.blueCore}╠${NetworkVisualization.pad(`╣ ${colors.orangeNeon}SOLVED MAZES${colors.blueCore} ╠`, 148, '═')}╣${colors.reset}`);
      this.logFunction(`${colors.blueCore}╠${NetworkVisualization.pad('╩══════════════╩', 148, '═')}╣${colors.reset}`);
    }

    this.logFunction(`${colors.blueCore}║${NetworkVisualization.pad(' ', 148, ' ')}║${colors.reset}`);
    this.logFunction(`${colors.blueCore}║        ${colors.neonSilver}${NetworkVisualization.pad(`Current generation: ${this.currentBest?.generation || 0}`, 140, ' ', 'left')}${colors.blueCore}║${colors.reset}`);
    this.logFunction(`${colors.blueCore}║        ${colors.neonSilver}${NetworkVisualization.pad(`Best fitness so far: ${this.currentBest?.result.fitness.toFixed(2) || 0}`, 140, ' ', 'left')}${colors.blueCore}║${colors.reset}`);
    this.logFunction(`${colors.blueCore}║${NetworkVisualization.pad(' ', 148, ' ')}║${colors.reset}`);
    ...
    if (this.solvedMazes.length > 0) {
      ...
      for (let i = this.solvedMazes.length - 1; i >= 0; i--) {
        ...
        const solvedMazeVisualization = MazeVisualization.visualizeMaze(solved.maze, endPos, solved.result.path);
        const solvedMazeLines = Array.isArray(solvedMazeVisualization) ? solvedMazeVisualization : solvedMazeVisualization.split('\n');
        const centeredSolvedMaze = solvedMazeLines.map(line => NetworkVisualization.pad(line, 150, ' ')).join('\n');
        ...
        const optimalLength = MazeUtils.bfsDistance(MazeUtils.encodeMaze(solved.maze), startPos, exitPos);
        ...
        this.logFunction(`${colors.blueCore}║       ${NetworkVisualization.pad(`${colors.neonSilver}Path efficiency:${colors.reset} ${optimalLength}/${pathLength} (${efficiency}%)`, 140, ' ', 'left')} ${colors.blueCore}║${colors.reset}`);
        this.logFunction(`${colors.blueCore}║       ${NetworkVisualization.pad(`${colors.neonSilver}Path overhead:${colors.reset} ${overhead}% longer than optimal`, 140, ' ', 'left')} ${colors.blueCore}║${colors.reset}`);
        this.logFunction(`${colors.blueCore}║       ${NetworkVisualization.pad(`${colors.neonSilver}Unique cells visited:${colors.reset} ${uniqueCells.size}`, 140, ' ', 'left')} ${colors.blueCore}║${colors.reset}`);
        this.logFunction(`${colors.blueCore}║       ${NetworkVisualization.pad(`${colors.neonSilver}Cells revisited:${colors.reset} ${revisitedCells} times`, 140, ' ', 'left')} ${colors.blueCore}║${colors.reset}`);
        this.logFunction(`${colors.blueCore}║       ${NetworkVisualization.pad(`${colors.neonSilver}Steps:${colors.reset} ${solved.result.steps}`, 140, ' ', 'left')} ${colors.blueCore}║${colors.reset}`);
        this.logFunction(`${colors.blueCore}║       ${NetworkVisualization.pad(`${colors.neonSilver}Fitness:${colors.reset} ${solved.result.fitness.toFixed(2)}`, 140, ' ', 'left')} ${colors.blueCore}║${colors.reset}`);
        this.logFunction(`${colors.blueCore}╠${NetworkVisualization.pad('╦════════════════╦', 148, '═')}╣${colors.reset}`);
        this.logFunction(`${colors.blueCore}╠${NetworkVisualization.pad(`╣ ${colors.orangeNeon}WINNER NETWORK${colors.blueCore} ╠`, 148, '═')}╣${colors.reset}`);
        this.logFunction(`${colors.blueCore}╠${NetworkVisualization.pad('╩════════════════╩', 148, '═')}╣${colors.reset}`);
        this.logFunction(`${colors.blueCore}║${NetworkVisualization.pad(' ', 148, ' ')}║${colors.reset}`);

        this.logFunction(NetworkVisualization.visualizeNetworkSummary(solved.network));
