...
// Exports: runMazeEvolution, printNetworkStructure

import { Neat, Network, methods } from '../../../src/neataptic';
import seedrandom from 'seedrandom';
import { encodeMaze, findPosition } from './mazeUtils';
import { simulateAgent } from './mazeMovement';
import { IFitnessEvaluationContext, IRunMazeEvolutionOptions } from './interfaces';
import { defaultFitnessEvaluator } from './fitness';
 ...
 */
export async function runMazeEvolution(options: IRunMazeEvolutionOptions) {
  ...
  const neat = new Neat(inputSize, outputSize, neatFitnessCallback, {
    ...
    provenance: Math.max(1, Math.floor(popSize * 0.1)), // New random individuals per generation
  ...
  while (true) {
    // === Evolutionary Loop ===
    // 1. Darwinian evolution: evolve the population (suffle genomes)
    ...
    // 3. Baldwinian refinement: further train the fittest individual for evaluation only. (winner goes to college)
...
/**
 * Utility function to print the structure of a given neural network to the console.
 *
 ...
 */
export function printNetworkStructure(network: Network) {
  ...
  console.log('Activation functions:', network.nodes.map(n => n.squash?.name || n.squash));
  console.log('Connections:', network.connections.length);
  const recurrent = network.connections.some(c => c.gater || c.from === c.to);
