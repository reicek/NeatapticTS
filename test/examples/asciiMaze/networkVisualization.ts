...
export class NetworkVisualization {
  ...
  static #isNodeCandidate(
    node: unknown
  ): node is INodeWithConnectionInfo {
  ...
  const satisfiedIdentifiers = new Set(inputIdentifiers);
    ...
    while (remaining.length > 0) {
      ...
      for (const node of remaining) {
        ...
        const dependenciesSatisfied = incomingConnections.every((connection) => {
  ...
  static #categorizeNodes(network: INetwork): CategorizedNodesResult {
    ...
    const visualizationByNode = new Map<INodeWithConnectionInfo, IVisualizationNode>();
    ...
    nodesCandidate.forEach((candidateNode, index) => {
      ...
      const visualization = NetworkVisualization.#toVisualizationNode(node, index);
  ...
  static #computeConnectionCounts(
    ...
    const hiddenIdSets: Set<number>[] = hiddenLayers.map((layer, layerIndex) =>
      new Set(
        layer.map((node) =>
          NetworkVisualization.#resolveVisualizationId(
            node,
    ...
    const hiddenLayersCanonical: HiddenLayerCollection = hiddenLayersRaw.map(
      (layer, layerIndex) =>
        layer.map((node, nodeIndex) =>
          ensureVisualizationForNode(
            node,
            layerIndex * 10_000 + nodeIndex
          )
        )
    );
