...
describe('enhanced adaptive mutation strategies', () => {
  const fitness = (net: Network) => {
    ...
    const connections = (net as unknown as { connections: Array<{ enabled?: boolean }> }).connections;
    const conns = connections.filter((connection) => connection.enabled !== false).length;
    return conns;
  };
  ...
  const finalRates = (neat.population as unknown as GenomeLike[]).map((genome) => genome._mutRate);
