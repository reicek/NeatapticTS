 ...
 */
export function createIdenticalNetworks(count: number, inputs: number, hiddens: number, outputs: number): Network[] {
 ...
 */
export function createDataset(type: 'XOR' | 'AND' | 'OR' | 'SIN', size: number = 4): { input: number[], output: number[] }[] {
  switch (type) {
    case 'XOR':
      return [
        { input: [0, 0], output: [0] },
...
export function expectWarning(fn: () => void, expectedWarning: string): void {
  ...
  expect(mockWarn).toHaveBeenCalledWith(expect.stringContaining(expectedWarning));
}
 ...
 */
export const createTestingSamples = (count: number = 10): { input: number[], output: number[] }[] => {
...
export const compareNetworks = (
  ...
  expect(net1.nodes.length).toEqual(expectIdentical ? net2.nodes.length : expect.any(Number));
